...
const bandOf = (d: string) => d.match(/APERTURE — ([A-Z]+):/)?.[1] ?? "?";

/* ── THE POLARITY ───────────────────────────────────────────────────────────────────────────── */
{
  const walk = [-8, -4, -2, 0, 5, 9].map((r) => [r, bandOf(at(r))] as const);
  console.log(`     (${walk.map(([r, b]) => `${r}:${b}`).join("  ")})`);
  check("terror still tunnels", bandOf(at(-8)) === "TUNNELLED");
  check("ordinary strain is where the noise is", bandOf(at(-4)) === "INTRUSIVE");
...
}
{
  ...
  check("under strain the attention will not stay put", /will not stay where it is put/.test(strained));
...
{
  const d = at(-4);
  check("a fragment is named as correct", /A fragment is correct and a whole sentence is usually wrong/.test(d));
  check("nothing joins one noticing to the next", /No conjunction joins one noticing to the next/.test(d));
  check("…and nothing explains the jump", /with nothing explaining the jump/.test(d));
  check("two facts about a thing, not three", /Two facts about a thing and no third/.test(d));
  check("naming beats describing", /name it rather than describing it/.test(d));
  check("it may ask and not answer", /ask itself something and not answer it/.test(d));
  check("it may come back late having missed a line", /come back late, mid-exchange, having missed a line/.test(d));
  ...
  check("and the distraction is never named as one", /Never write that the player noticed, was distracted/.test(d));
}
check("at ease the grammar relaxes with it", /it can be a whole sentence and settle for a moment/.test(at(6)));
