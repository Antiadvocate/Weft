...
/**
 * FACTS â€” the fidelity layer. Two jobs, both deterministic and zero-token:
 *
 ...
 *    episodics + reflected beliefs â€” both of which pass through a small model's
 *    paraphrase, where proper nouns silently mutate (Seattle â†’ Portland), and both
 ...
 *    (This is the attribution-verification idea from grounded-generation research â€”
 *    RARR-style "does the source actually say this" â€” done as a substring check
...
export function properNouns(s: string): string[] {
  ...
  const re = /\b([A-Z][a-zA-Z'â€™-]+(?:\s+[A-Z][a-zA-Z'â€™-]+)*)\b/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(s))) {
    const run = m[1];
    // sentence-initial SINGLE capitalized words are usually just sentence case, not names â€”
    ...
    const words = run.split(/\s+/).filter((w) => !CAP_STOP.has(w.replace(/[â€™']s$/, "")));
...
const norm = (s: string) => s.toLowerCase().replace(/[â€™']/g, "'");
...
export function groundMemoryContent(
  ...
  // an anchor that genuinely appears in the source is the strongest evidence â€” prefer content as-is
...
/** LEDGER QUALITY GATE â€” a fact must stand alone, cold, to a stranger reading the ledger.
 ...
 *  Deterministic on purpose â€” the gate itself must never hallucinate. */
export function factGate(content: string): { ok: boolean; why?: string } {
  ...
  if (["she", "he", "they", "it", "her", "his", "their", "she's", "he's", "they're", "it's"].includes(first))
    return { ok: false, why: "bare-pronoun subject â€” no one can tell who this is about" };
  if (f.split(/\s+/).length < 6) return { ok: false, why: "fragment â€” too short to mean anything cold" };
  if (/"/.test(f) || /^['"â€œ]/.test(f)) return { ok: false, why: "quoted dialogue â€” paraphrase the claim; quotes belong in the anchor" };
  if (/\b(currently|right now|at this moment|for now|is making (her|his|their) way)\b/i.test(f))
    return { ok: false, why: "transient state â€” true today, gone tomorrow; that's a memory" };
  return { ok: true };
}

/** Fuzzy near-duplicate merge: high content-word overlap keeps ONE entry â€” the more detailed. */
...
export function addFact(mem: CharMemory, fact: string, turn: number, quote?: string, source?: import("./types").MemorySource): boolean {
  ...
  if (!gate.ok) { console.warn(`[facts] rejected: "${f.slice(0, 60)}" â€” ${gate.why}`); return false; }
 ...
 *  source text nor the whitelist â€” the cheap model invented a specific. */
