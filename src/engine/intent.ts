...
// â”€â”€ INTENT PASS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// ACTUALLY doing this beat â€” the lie they'll tell, the want they're hiding, the
// feeling they're withholding â€” authored from THEIR OWN state (drive, agenda, edges,
...
// it should:
//   â€¢ surface  â†’ the NARRATOR renders it as deniable behavior (the player sees the
//                bitten teeth, not the lie).
//   â€¢ truth    â†’ the BOOKKEEPER records it ("lied to Rabi about the shipment", builds
//                the deceit trait) â€” it reads from truth, not from the opaque prose.
//   â€¢ tell     â†’ an optional deniable behavioral leak the narrator MAY show (a flicker,
...
// runs on the cheap model. Most turns it fires for zero or one character. No stakes â†’
// no call â†’ zero added cost.
...
export interface NpcIntent {
  char_id: string;
  name: string;
  surface: string;   // what they let show â€” for the narrator
  truth: string;     // what's actually true (lie/hidden want/withheld feeling) â€” for the bookkeeper
  tell?: string;     // optional deniable behavioral leak â€” the narrator may show it, the player may read/misread it
 ...
 *  synchronous gate â€” no model call. Returns the reason (for the prompt) or null. */
function stakesFor(state: SaveState, id: string): string | null {
  ...
  if (e) {
    ...
    if (e.trust <= -20) reasons.push(`distrusts the player (${e.trust}) â€” may conceal or mislead`);
  }
  ...
  // clenched + low openness â†’ likely withholding rather than open
  if (cond.psyche.relaxation <= -3) reasons.push(`clenched (openness ${cond.psyche.relaxation}) â€” more likely to mask than to show plainly`);
  return reasons.length ? reasons.join("; ") : null;
}

const INTENT_SYSTEM = `You author the PRIVATE, TRUE intent of a single character for one beat of a story â€” what they are ACTUALLY doing beneath what they let show. This is the character's own truth, drawn ONLY from who they are and their situation, NEVER from the player's private thoughts (you are not given those, and the character cannot know them).

Return ONE strict JSON object, nothing else:
{"surface":"what this character lets others SEE and HEAR this beat â€” their outward behavior, words, manner (this is what gets dramatized)","truth":"what is ACTUALLY true underneath â€” the real want, the lie and what it conceals, the feeling being withheld. If the surface is honest, truth restates the genuine state plainly. If they lie, truth names the lie AND the fact being hidden.","tell":"OPTIONAL â€” a small, deniable behavioral leak of the truth (a flicker, a too-quick reply, a hand that stills). Something an observer COULD read, or could miss/misread. Omit if they mask cleanly.","lying":true or false}

Rules:
- The character acts from THEIR nature, agenda, and feelings â€” sovereign, not in service of the player. They pursue their own want this beat.
- surface and truth may match (an honest, open character, or one who is simply, plainly feeling what they feel) or diverge (a liar, someone hiding desire, someone saving face). Divergence is ONE tool, not the default â€” use it only when the character's state actually supports a concealed truth. A resigned, hurt, or wary person is usually just that underneath; do not manufacture a hidden agenda, a secret scheme, or a dark reading of the player where the state shows only ordinary feeling. Most beats, surface and truth are close and lying is false.
- CALIBRATE TO THE STATE â€” the truth must be PROPORTIONAL to the character's actual disposition (given as warmth/trust with their plain-language meaning). Mild negatives are mild: warmth slightly below zero is "a little hurt, a little guarded", NOT terror; low trust is "cautious, watching", NOT conviction that the player is a monster. Do NOT escalate a wary or resigned character into someone secretly certain the player is a manipulator, a monster, a hollow shell, or a danger â€” that is invention, and it poisons how the character is played. Only write fear, hatred, or a dark verdict when the warmth/trust and history genuinely support that intensity. If the numbers say "mildly hurt but still cares," the truth is mildly hurt, full stop.
- The character reacts to what the player actually SAID and DID this beat and to their real history â€” never to a sinister interpretation the state doesn't justify. If nothing hostile has actually happened, the character is not secretly seething about it.
 ...
 *  Fires zero calls when nobody has stakes. One cheap call per staked NPC (usually 0â€“1). */
export async function runIntentPass(state: SaveState, playerAction: string): Promise<NpcIntent[]> {
  ...
  // What the player AUDIBLY said / VISIBLY did â€” the only player input an NPC may react to.
  ...
  const results = await Promise.all(staked.map(async ({ id, reason }) => {
    ...
    const ctx = [
      ...
      e ? `Toward the player: warmth ${e.warmth}, trust ${e.trust}${e.attraction !== undefined ? `, desire ${e.attraction}` : ""}${e.roles?.length ? `, roles ${e.roles.join("/")}` : ""} â€” ${dispositionCue(e.warmth ?? 0, e.trust ?? 0)}${belief ? `. WRONGLY BELIEVES: ${belief}` : ""}.` : "They barely know the player.",
 ...
 *  receive `truth` â€” it renders deniable behavior so the player sees the surface and reads (or
...
export function intentForNarrator(intents: NpcIntent[]): string {
  ...
  const lines = intents.map((i) => {
    const bits = [`${i.name} â€” shows: ${i.surface}`];
    if (i.tell) bits.push(`may leak (deniable, the player could catch or miss it): ${i.tell}`);
    return `- ${bits.join("; ")}`;
  });
  return `\n\n=== WHAT PRESENT CHARACTERS LET SHOW (render as behavior; do NOT state their hidden reasons â€” the player reads them like anyone reads a face) ===\n${lines.join("\n")}`;
}

/** Format the intents for the BOOKKEEPER: the TRUTH. This is what it records â€” the lie and what
 *  it conceals, the hidden want â€” so memory and traits are built from what really happened, not
...
export function intentForBookkeeper(intents: NpcIntent[]): string {
  ...
  return `\n\n=== GROUND TRUTH OF PRESENT CHARACTERS THIS TURN (authoritative â€” record memories/facts/traits from THIS, not from the prose, which deliberately hides it; e.g. a lie becomes a memory "lied to the player about X" for the liar, and may build a deceit trait) ===\n${lines.join("\n")}`;
}
