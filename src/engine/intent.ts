...
- THE PERSON IS THE SUBJECT OF EVERY SENTENCE IN "truth". She wants, she is afraid, she has decided, she cannot say. When a sentence instead begins with a thing — the accusation, the invitation, the laugh, the silence, her own bewilderment — you have stopped reporting a mind and started describing the scene's effect on her from outside, which is the narrator's job and not this field's. Written from inside her the same beat reads: she wants to say yes and is frightened of what it costs her.
- PLAIN WORDS. This field is read by the bookkeeper and it is the character's own mind, so it is written the way a person thinks and not the way a novel narrates. No figures of speech, no image standing in for a feeling, no clause balanced against another, and no abstraction handed a verb of its own. Say what she wants, say what she is scared of, name the person or the thing it is about, and stop. A nineteen-year-old and a sixty-year-old magistrate do not use the same words about the same trouble; use the words on that person's own card.
- Keep each field to one or two tight sentences, naming what is actually there — a person, an object, an act, a thing owed. TWO SENTENCES IS THE CEILING and one is often enough: these are stored and rendered at a fixed width, and a third sentence is cut off, which reads as the thought being lost mid-way rather than the field having a limit.
- NO QUOTATION MARKS ANYWHERE IN YOUR OUTPUT — in surface, in truth, or in tell. A quoted line here becomes a sentence the player is later shown as something the character said, when it was never spoken in the story at all — the surest way to make the whole system look like it is fabricating.
...
- TAKE THE YES. If the player has now given the thing this character was waiting for, the want is MET. Write what a person does after they get what they asked for — relief, awkwardness, a new want, wanting more of it, hands with nowhere to go. Do not write them re-opening it, discounting it for how it arrived, or moving on to a fresh test. That is the same failure wearing a new sentence.`;
