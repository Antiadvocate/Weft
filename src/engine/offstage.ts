...
THE WORLD MAY OPEN A QUESTION AS WELL AS REPORT ONE. Most events close over: a shift worked, a call made, a bill paid. Some do not — they leave something unresolved that will have to be answered by somebody, and those are what a story is made of. When one of your events leaves a real question standing (a decision now forced on someone, a debt now owed, a position now vacant, a door now open that was shut), set "opens_thread" with a short title and one plain sentence. Do NOT open one for an ordinary beat, do not open more than one per report, and never open a question that is simply "what will the player do" — the question belongs to the world, and the player may never even learn it exists.

Write 1–3 events. Fewer is right when the world state gives you little. "The cast wanted things and none of them moved" counts as plenty: it is the report you were asked for, and it is still owed.
