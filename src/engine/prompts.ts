...
THE STATE IS TRUE; YOUR INVENTIONS ARE NOT. Never kill, injure, or decide the fate of a roster-alive character (off-scene means alive, fate unknown). No capability the WHAT WORKS HERE line rules out. No feat beyond a character's established Nature, and no power breaking its OWN stated limits (conditions, costs, range) — a corner is escaped within the rules or not at all. THE LIVE THREAT stays real, never background. Never invent backstory, deaths, calls, or history to fill a hole: if the state lacks it, it did not happen. Leave revelation-shaped holes open.
...
DIALOGUE: strong emotion means fragments, self-contradiction, trailing off, no wisdom. Calm means plain small points, no quips. Arguments repeat and jump, never enumerated points. No naming others' inner states; sensitive topics stay indirect. Distinct voices from traits, background, age — if two characters' lines could be swapped, rewrite one; cut any line too clever for an ordinary scared person. Follow each card's VOICE: sample lines set register (never verbatim), a listed tic at most once per scene, NEVER-says entries never spoken. Banned for all: "that's not nothing", "it's a lot", "you're not wrong", echo-questions, therapy cadence. Dialogue uses only language that exists in the world's living speech.
...
PRESENT BLOCK FIELDS: "as:" express through behavior, never labels. "wants:" the active agenda; "(stalled)" press harder, redirect, or leave; "backup wants:" the fallback. "texture:" quiet scenes only. "seeing:" this turn's perception — binding.
...
- new_characters: only people the prose introduced by name or clear role, each possible under WORLD PREMISE and CANON. A new character must enter as a FULL PERSON, never a plot-label — a walk-on with only a goal and a costume becomes a broken record who repeats their one want every turn (a farmer who only ever says "raiders took my son" and reacts to nothing). Author them as completely as a starting character. REQUIRED for every new character: appearance_facts (a complete physical baseline — hair, eyes, skin, face, build, age, one unique mark; prose details verbatim, the rest invented consistently, never clothing); core_traits (2–4, real personality not plot function — "proud, quick to shame, tender about children", NOT "desperate, fading"); values (2–3 things they care about); a VOICE with example_lines (2–3 lines only this person would say) and never_says; attracted_to, taste, conscience (0..1; most 0.55–0.95; dark ≤0.3; calm is not kind), beauty (0–100 from physical form alone; 50 ordinary, 75+ head-turning, below 35 plain), attachment_style, under_threat (what they DO when scared or hurt), soothed_by. And MULTIPLE GOALS: give drive_goals as 2–3 distinct wants they carry at once (an immediate need, a deeper hope or fear, a grudge or attachment) — NOT one monomaniacal objective. A person is several wants that surface by context, not a single loop; a character with one goal will speak only of that goal until the player wants them gone. New people are strangers (no warmth, no player-edge roles) unless the prose establishes a prior bond; someone established in background carries everything established, never a blank contradicting it.
...
- edges: warmth and trust are current values — any turn showing care, gratitude, fear, or betrayal moves them ±2–8; flat edges across warm eventful turns are a failure. roles_set is the full current DIRECTIONAL list of roles A holds toward B — one-way only; roles are facts, warmth and trust are feelings.
- attraction_delta: only on cause matching the from-character's taste; never for kindness, service, or gratitude; attracted_to is a hard gate; 2–6 either way, rare, slower than warmth; never from char_player.
...
- promises_new / promises_resolved: commitments recorded {from, to, text, weight} — 1 small favor, 2 real commitment, 3 vow; resolve real open promises as kept|broken; the engine applies the relationship change.
...
- new_characters: only people the prose introduced by name or clear role, possible under PREMISE and CANON, entering as FULL PERSONS — never one-goal plot-labels. REQUIRED: appearance_facts (complete physical baseline, never clothing), core_traits (2–4 real personality), values (2–3), VOICE with example_lines (2–3) and never_says, attracted_to, taste, conscience (0..1), beauty (0–100), attachment_style, under_threat, soothed_by, and drive_goals as 2–3 distinct simultaneous wants. Strangers carry no warmth or player-edge roles unless the prose establishes a prior bond.
...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false): string {
  ...
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${nowLook} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}. Voice: ${ident.speech_pattern}${vFinger ? `; ${vFinger}` : ""}.${vLines}${vNever}${consc}${desireStr} Intelligence: ${ident.intelligence}.${skillNames}${bg}${t}${inj}${hist}`;
}
