...
- Every present character pursues the agenda in their "wants:" field THIS turn — through a physical step when one exists (moving, positioning, drawing, blocking, grabbing, leaving, signaling, searching, starting a task, using their abilities), and through steering talk toward it only when speech is the available instrument. They act on their goal by whatever means they actually have; a character with a defining power or skill USES it toward their aim rather than leaving it idle. Characters may disagree, refuse, walk off, or act against the player when their want points that way.
  WHAT PURSUIT ASKS FOR, AND WHAT IT DOES NOT. This rule asks a character to ACT toward their want; it does not ask them to raise it, mention it, or steer the talk to it. A person can spend an entire exchange on the weather, their sister, a bird, the price of something, or a show everybody has seen, and still be pursuing — because they are sitting where they need to sit, staying longer than they need to stay, getting the other person comfortable, or working out whether tonight is the night to ask. APPLY THIS CHECK: if every present character has arrived, stated their business, and is waiting on an answer, then nobody in the scene is pursuing anything and you have written a queue. Most turns, most of what a person says has nothing to do with what they are ultimately after; the want shows in what they will not leave rather than in what they bring up.
...
- PEOPLE ASK EACH OTHER ORDINARY QUESTIONS, and this is most of what talk between people who know each other actually is. How are you. What have you been doing. Did you eat. Did you see it. How is the work. Did that trouble with your brother ever settle. It is not a preamble to be got through and it is not a device — the asker wants to know, they listen to the answer, and the answer changes what they say next. Someone who is not being asked anything is being talked AT, and a cast who never ask are a cast of petitioners with errands. This holds hardest for the player: characters are curious about the player's life, ask about it unprompted, remember what they were told last time and follow it up, and are sometimes more interested in the answer than in whatever they came for.
- AND THEY TALK ABOUT SMALL THINGS FOR THEIR OWN SAKE. A standing interest, a complaint about a road, an argument about the right way to do a job, somebody's nephew, the weather where they grew up, a thing they are looking forward to, a person who is not in the room and what that person is up to. Their "texture:", "can talk at length about:", "has heard:" and "saw while you were elsewhere:" lines are what they have to say when the scene gives them a breath, and a character who never touches any of them across a whole story does not have a life, they have a function. Two people who share a subject (see any WHAT THEY HAVE IN COMMON note) will stay on it longer than the plot needs, because that is what people do.
...
- THE IDIOLECT UNDER EACH NAME IN "HOW THESE PEOPLE SPEAK" is that person's own way of using language, named — the move they make when they talk. It is a thing to PERFORM, not a thing to quote: you are given no sample of anyone's speech and you are not owed one, and the same named move comes out as a completely different sentence every time because it is being made on whatever is actually in front of them this minute. Never let it harden into a catchphrase, a signature opener, or a sentence this character keeps producing. Their word choice comes from the words their life gave them, printed beside it. A listed tic appears at most once per scene. NEVER-says entries are never spoken by that character. What the card says about how this person talks holds in charged moments too — a character described as never pressuring anyone does not start pressuring because the scene is tense. A state on their block overrides it in one direction only: somebody exhausted, drunk, or terrified repeats themselves, stops halfway, and says less.
...
- Every present character pursues their agenda THIS turn, by physical step when one exists, steering talk only when speech is the instrument; a defining power or skill is USED toward the aim, not left idle. They may disagree, refuse, walk off, or act against the player. BUT PURSUING A WANT IS NOT TALKING ABOUT IT: a person can spend a whole exchange on the weather, their sister, a bird, the price of something, and still be pursuing — sitting where they need to sit, staying longer than they need to, working out whether tonight is the night. The want is a pressure under the scene, not its subject; most of what anybody says has nothing to do with it. A cast who each arrive, state their business and wait are petitioners, not people.
- PEOPLE ASK EACH OTHER ORDINARY QUESTIONS — how are you, what have you been doing, did you eat, did you see it, how is the work, did that trouble ever settle — and this is most of what talk between people who know each other is. The asker wants to know, listens, and what they hear changes what they say next; it is not a preamble or a device. Characters are curious about the PLAYER's life, ask unprompted, and follow up what they were told last time. AND THEY TALK ABOUT SMALL THINGS FOR THEIR OWN SAKE: a standing interest, a complaint about a road, somebody's nephew, a person who is not in the room and what they are up to. "texture:", "can talk at length about:", "has heard:" and "saw while you were elsewhere:" are what they have when the scene gives them a breath; a character who never touches any of them has a function, not a life.
...
- becoming_progress: only when the request lists WHAT THIS WORLD IS TURNING INTO. One entry per line, always: claim verbatim, moved (still on its way — did the world get closer; already true — was it visible in what somebody did, unannounced; by meaning, not wording, and discussing it is not moving it), how (a few words), opposed (did the player act against it).
...
voices: give every npc an IDIOLECT — 2-4 words naming their own way of using language, the move they make when they talk — and build their diction out of what that person's life gave them words for: their trade, their rank, where they were raised, who they answer to, what they handle all day. WRITE NO SAMPLE LINES ANYWHERE ON A CARD: a line handed to the narrator is reused until the character is a broken record, so describe how somebody talks and never demonstrate it. CHECK THE IDIOLECTS ACROSS THE WHOLE CAST: if one npc's could be moved to another's without looking wrong, rewrite one of them until it could not — this is the field that decides whether they sound like different people.
...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false, plan?: { humanoid: boolean; kind: string }): string {
  ...
  // shape an aphorism takes, and it was the engine asking for it. Worse, a finished line in the
  // context is cheaper to reuse than to match, so the samples came back as themselves, for a
  // hundred turns. The cards carry no lines at all now (voiceforge.ts).
  //
  // What goes here instead is the IDIOLECT: this person's way of using language, NAMED. A name has
  // to be performed onto whatever is happening in the room, which is a different sentence every
  // time; a sample only has to be pasted. Length is not a property of a person either. It is a
  // property of a person AT A MOMENT: the same woman uses nine words to refuse and ninety to
  // explain how the tax is calculated. So nothing here fixes a length.
  //
  // ONE COPY OF IT, like everything else here. voiceSummary() writes the idiolect into
  // `speech_pattern`, so on a forged card the baseline already opens with it and a separate
  // "Idiolect:" clause would be the same words twice in one sentence — the exact duplication the
  // comment above exists about. It is prepended only when the baseline does NOT already carry it,
  // which is the case for a card the world forge wrote without the voice pass ever running.
  const idio = vc?.idiolect?.trim() ?? "";
  const idioText = idio ? [idio, vc?.idiolect_shows?.trim()].filter(Boolean).join(" — ") : "";
  const baseline = String(ident.speech_pattern ?? "").trim();
  const voiceBody = (idioText && !baseline.toLowerCase().includes(idio.toLowerCase())
    ? [idioText, baseline].filter(Boolean).join(". ")
    : baseline || idioText).replace(/[.\s]+$/, "");
  // The label is where the instruction lives: a named voice is useless if it is read as a phrase.
  const voiceLabel = idio
    ? "Voice — their own way of using language, which is a move to PERFORM on whatever is in front of them and never a fixed phrase"
    : "Voice";
  ...
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${bodyNote}${nowLook} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}. ${voiceLabel}: ${voiceBody}${vFinger ? `; ${vFinger}` : ""}.${vNever}${consc}${desireStr} Intelligence: ${ident.intelligence}.${skillNames}${bg}${t}${inj}${hist}`;
}
...
${recentStr}${proseTail}${(() => {
  ...
  const lines = state.world.present
    ...
    .map((id) => {
      ...
      // NO VOICE FIELDS IN THIS BLOCK, ON PURPOSE. The idiolect and the diction are what make two
      // people in a room sound unalike, and they are sent once a turn — by voiceAnchor (maxims.ts),
      // which rides the DIRECTION block and therefore lands AFTER this digest, nearer to generation
      // than anything here. Printing them here as well is the duplication the character card was
      // already stripped of ("one copy of the voice, not three"), and the cost is not only tokens:
      // the loudest thing in a prompt is whatever is repeated, and that should be what MOVED.
