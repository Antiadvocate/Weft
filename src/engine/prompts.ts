...
- LOCATION: the world has a fixed list of places, given to you under LOCATIONS. There are no others and none can be made. Use a place name from that list exactly, or the word "elsewhere" for anyone who is not in one of them. Rooms and corners inside a place ("the kitchen", "the yard", "the stairwell") are NOT locations — a character in Tessa's kitchen is at "Tessa's house". Set player_location whenever the player moves to a different place on the list.
...
Player carries: ${state.world.money || "—"}${(() => {
  ...
  return `\n\nLOCATIONS — the only places in this world:\n${list}\nThere are no others, and none can be added. Rooms and corners inside a place (a kitchen, a yard, a stairwell, a doorway) are things you describe in prose, not places people travel to. Someone who steps into the next room is still here.\n${away.length ? `\nNot in this scene: ${away.join(", ")}. They cannot see or hear anything that happens here. Do not give them lines. Do not have them react to this, or know about it later. Someone arrives only if you write them arriving — and if you do, say so plainly: "Dana came in from the street." Someone leaves only if you write them leaving: "Lena set down her cup and went out." Never move a character silently.` : ""}`;
})()}
