...
export function deltaNote(state: SaveState, query: string): string {
  ...
  for (const id of ["char_player", ...state.world.present]) {
    ...
    const bits = [
      `${c.name} [${id}]${c.pronouns ? ` · ${c.pronouns}` : ""}${c.knows_player_name === false ? " · DOES NOT KNOW YOUR NAME" : ""} — mood ${cond.psyche.mood || "even"}; seeing: ${describeOpenness(cond)}`,
