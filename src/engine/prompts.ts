...
- OPEN LOOPS: a memory that leaves something unfinished for that character MUST carry scheduled_time — an answer owed, a message interrupted or partly given, a summons not yet obeyed, an arrival expected, a promise not yet kept, a question asked and not answered. Give the in-world time it comes due, or "unresolved" if there is no clock on it. This is what keeps the thing they are still waiting on at the front of their mind; a character whose open business is unmarked will behave as though it never happened and re-open a scene they already played. Clear it by emitting the same memory's resolution when it is finally settled.
