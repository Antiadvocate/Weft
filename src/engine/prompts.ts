...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false, plan?: { humanoid: boolean; kind: string }, anatomy = ""): string {
  ...
  // ── A SEVEN-YEAR-OLD IS NOT A SHORT ADULT ────────────────────────────────────────────────────
  //
  // The card prints an age and the dialogue rules mention it in a list of things a voice comes from,
  // and that is the whole of it. A save's seven-year-old, on meeting a dog: "That's called
  // heterochromia. I read about it in Mama's garden book even though it's not about gardens, it was
  // on the back page where they put the animal facts." Then, a beat later, "I'm being very slow so
  // he knows I'm not a threat. This is how you meet a dog." That is an adult explaining a procedure,
  // with a child's vocabulary painted on.
  //
  // The failure is specific and it is not about vocabulary: children do not narrate their own
  // method, do not hold a subject across several sentences to develop it, and do not summarise. So
  // the constraint is written as sentence mechanics rather than as "sound younger", which produces
  // baby-talk and nothing else.
  const yrs = typeof ident.age === "number" ? ident.age : undefined;
  const childNote = yrs !== undefined && yrs <= 12 && !nonHuman
    ? ` AGE IS BINDING ON HOW ${(ident.name || "they").toUpperCase()} TALKS — ${yrs} is a child, not a small adult.${
        yrs <= 2
          ? ` Single words and two-word pairs, the same few over and over, mostly naming what they want or see. No sentences. No questions with a verb in them.`
          : yrs <= 6
          ? ` Short sentences, one idea each, joined with "and" when they run on. They announce what they see and what they want. They do not explain, compare, summarise, or say what something is called.`
          : ` One idea per sentence, and they jump between ideas rather than developing one. They can know a fact and be proud of it, but they state it and move — a child does not hold a subject for a paragraph, does not narrate their own method ("I'm doing it slowly so that..."), does not recap what somebody just said back to them, and does not count up what they have been promised. Where an adult would explain, they simply do the thing, or ask.`
      } What they DO is not limited by this — a child can be brave, cruel, exactly right, and better at reading a room than the adults. It is the sentences that are a child's.`
    : "";
  ...
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${bodyNote}${anatomyNoteText}${childNote}${nowLook} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}. Voice: ${ident.speech_pattern}${vFinger ? `; ${vFinger}` : ""}.${vLines}${vNever}${consc}${desireStr} Intelligence: ${ident.intelligence}.${skillNames}${bg}${t}${inj}${hist}`;
}
