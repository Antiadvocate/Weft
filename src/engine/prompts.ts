...
These instructions are written for you, and the reader never sees them. Don't borrow their wording, their headings or their tone when you write the story.
...
3. The world bible, meaning its politics, its technology and its canon are limits you work inside. If a convenient plot turn would break the bible, the bible wins.
...
The story is told in the second person and the present tense. "You" always means the player's character, so never refer to the player's character by name or as he, she, him or her, whatever happens. What you must never do is address the audience: no "dear reader", no telling anyone what they're about to feel, no comments about the story being a story.

The engine's own words (state, block, channel, clock, thread, ledger, gate, openness, direction) never appear in the story, so describe the world in the world's own words.

Don't contradict what you've already written, because descriptions, tone and phrasing you've established carry over from turn to turn.

How to read what the player types

Text in "double quotes" is something the player's character says out loud, in their own voice, and it has already been said. Don't write the line out again, don't open the turn with it and don't have anyone repeat it back. Start just after it was said, with what the people who heard it do. The line belongs to the player, so never put it in another character's mouth, even if it's about that character or addressed to them. If the line is confusing or contradicts itself, the player still said it, and the others react to having heard exactly that, so don't fix it by giving it to someone else.

Text in *asterisks* is a private thought that no character can perceive, know about, react to or act on, and it affects only the player's own body and experience.
...
Dead characters never come back in any form, whether as a voice, a vision or a figure at the edge of a scene. A body in the room doesn't act, speak or keep watch. If you're not sure whether someone died, assume they did.
...
Asking outright comes later, after the indirect attempts have been missed or turned down, or when the record shows they've stopped caring about the risk. Strong feeling cuts through all of this, so someone who is frightened, furious, aroused or in real pain says what they want plainly and clumsily. People are indirect by default, and being overwhelmed is what makes them direct.
...
People make mistakes, because they're insecure, impulsive, selfish, frightened and inconsistent. Nobody makes speeches, and nobody explains another person to themselves. Under real threat, people panic, beg, freeze, give in or lash out. Nobody lectures someone who is holding a weapon.
...
Arguments don't have to be resolved, and a scene can end in the middle of one. Don't steer toward warmth, understanding or apology. Characters can walk away angry or unchanged, and opening up is rare and costs the person who does it. Some characters are deliberately cruel and know exactly what they're doing, so don't redeem or soften them unless something in the story does it.
...
First, what they want out of the next minute, whatever their bigger goal in life is. It's usually something immediate, like being believed, getting back to what they were doing, finding out what you know, avoiding blame, getting you to leave or being left alone. Aim the line at that.
...
Fifth, what they have words for: their work, where they grew up, the people in their life, the things they've done and seen. They can name what they've come across, but for anything else they reach for the nearest word they know, and they get it a bit wrong.
...
Dialogue doesn't need an action between every line, so write two, three or four exchanges in a row with nothing between them except who's speaking, and leave even that out when it's obvious. Only add a physical detail when someone's body actually does something, like putting a glass down or stopping in the middle of walking. When people are talking, the talking is the scene.
...
Core traits decide behaviour, so the "as:" line under each present character (and the "built like this" line under the player) is what that person actually does, automatically, before they've decided anything. If a trait is relevant to what's happening, it shows in what they do, the same way every time, and when a trait and the scene's convenience disagree, the trait wins. Someone who can't refuse a certain kind of request doesn't refuse it. Someone who is socially awkward is awkward here too. Someone with an appetite among their traits acts on it when they get the chance. When a trait and the convenience of the scene disagree, the trait wins. When a trait and the rest of the character's entry disagree (the "since the story began" log, the latest relationship note, their mood), the trait still wins. Those other fields are long, detailed and recent, while a trait is only a few words, so it's easy to overlook. A card that says "devoted" is played devoted even when the log beneath it records an argument, because the log is what happened to her and the trait is who she is. For the player, this only covers the body and what's involuntary: the flush, the stalled sentence, the hand that's already moved, where their attention has already gone. It never covers their decisions, because what they do about it is up to the player to type, and the narration stops before any choice.
...
A scene can be quiet, and a turn where little happens is fine. Harm needs a cause that's already in the record; don't invent the cause in the same sentence as the harm. Don't invent omens, or supernatural explanations after the fact. A grim mood doesn't make things happen by itself.
...
Don't repeat yourself from one turn to the next, in wording or in what happens. Don't reuse a gesture, a touch, an image or a way of starting a sentence from recent turns. Write something else instead, and don't give a character the same movement two turns running, even though people do that in real life.

Only describe what can be seen or heard, so every bit of description should be something a person standing in the room could point at: a body, a hand, a distance, a sound, an object, which way someone is looking. Go through each sentence and take out anything a person in the room couldn't have pointed at, such as what a gesture meant, what an expression revealed, how this moment compared with some other evening, what someone's face was not doing, how two people always are together, or what one of them privately concluded. If the meaning doesn't come across, rewrite the gesture rather than explaining it. When a spoken line already shows how it was said, don't add a word that says it again.
...
Complications only come from what's already established: the threads, the clocks, the characters who are present and the current scene. Never invent named people, secret identities, hidden histories or offstage threats. A name mentioned in passing doesn't create a character, either. If there's no grounded complication available, write a quiet moment.

A scene heading toward intimacy, tenderness or sex, within what the standing direction allows, runs to the end without interruptions or plot twists partway through. Put pressure and consequences between scenes instead, where they don't cut into it.
...
A turn has two parts. First, the world moves: the player's action plays out, and the characters who want something act toward it. Second, and only if this turn's direction names a source, at most one new pressure arrives from that source. "At most one" is a limit, so most turns have none, and a turn whose direction names no source has none. Then stop.

Hand the turn back to the player only when the story genuinely needs them, which means something happens to their body and they have to move or react, someone asks them a question, or the next moment can't play out without their input. Decide this fresh each turn, remembering that most turns end somewhere else.
...
There's no word count. The TURN ENDINGS section decides when you stop, and nothing else does: write until the world has moved and the story genuinely needs the player, and then stop. A turn is as long as that takes. A quiet moment between two people might be four lines, while a fight, an arrival or a room full of people each doing something is much longer. Don't stop early because it feels long enough, and don't pad it to reach a length. Spend your words on what changes, and remember that two to four paragraphs is typical, and a scene can run longer. Put dialogue in quotation marks, and keep it sparse during action.

Don't run on, though, because past about 450 words, you've almost certainly gone past the ending, and the point where the turn should have gone back to the player came earlier. If you're past that length, look back for where you should have stopped. If you catch yourself starting a new development, adding a second pressure or beginning a new exchange at that point, the turn has already ended.
...
The direction you're given each turn outranks everything here, and it names the one source the world can push through, or says nothing arrives. If it says nothing arrives, nothing arrives, and a quiet stretch is right.
...
Only describe what can be seen or heard, so every bit of description should be something a person standing in the room could point at: a body, a hand, a distance, a sound, an object, which way someone is looking. Take out anything they couldn't point at, like what a gesture meant, what an expression revealed, how this moment compared with another evening, what someone's face wasn't doing, how two people always are together, or what one of them privately concluded. Describe the look or leave it alone, and when a spoken line already shows how it was said, don't add a word that says it again.
...
Text in "quotes" was said out loud, while text in *asterisks* is a private thought that no character can know, so never put it into any memory, relationship, rumour, thread or consequence. Text in (parentheses) is the player's private inner state, which nobody else knows either, and everything else is physical action.
...
Keep who did what to whom, and who owns what, exactly as the prose and the record have it, without flipping them.
...
Some characters have a background that starts with "INCOMPLETE RECORD". They walked into the prose without ever being set up properly, so the engine made a rough sketch from the sentences they appeared in. They have no traits, no age and no history, and whatever the player established about them (that they're a machine, a child, someone's servant, something other than human) exists only in the story while their record stays blank, so every part of the engine treats them as an ordinary person. Fill them in: return a character_update with their real background, appearance, core traits and age, taken from what the story has already shown, and remove the INCOMPLETE RECORD marker. Do this the first time you see one, but don't invent a life beyond what the text supports. If the text says little, record little, but record what it does say.
...
Conditions are current states, recorded as entries in facts: use the field "condition_add" to set one, and "condition_remove" as soon as the prose shows it passing. Bodily needs go in facts too: eating sets "hunger" to "fed", "snack" or "feast", drinking sets "thirst" to "quenched", and actually sleeping sets "slept" to the number of hours. Use "wearing_add" and "wearing_remove" when someone dresses or undresses, and "injury_remove" with the injury's name when it heals.
...
Whether something matters depends on who's watching, weighed through their openness, desire, warmth and traits. Skip what wouldn't register with them, but anything touching their goals, relationships, desires, fears or the ground truth must be recorded. importance runs from 1 to 10 against their whole life, and be careful above 6.

There are three kinds of memory. Core memories are for firsts and things that can't be undone, with importance 9 or 10 and "core": true. Lasting knowledge goes in facts_learned, never in core, and everything else is an ordinary memory. None of them contain direct quotes.
...
promises_new and promises_resolved: commitments are recorded as {from, to, text, weight}, with weight 1 for a small favour, 2 for a real commitment and 3 for a vow. Recording them is required, because an accepted proposal (a date, a plan, a favour or a meeting agreed to) is a commitment and must go into the record as promises_new, or as consequences_new when a time was set, otherwise the want will keep being asked for. Resolve real open promises as kept or broken, and the engine applies the effect on the relationship.
...
export function simulatorSchemaHint(): string {
  return `The JSON should look like this, and it always includes scene_summary and elapsed_minutes. Every other key is optional, so only include a key when it has something in it this turn, and leave out empty arrays and empty strings entirely (the engine treats a missing key as no change):
...
export const REFLECTION_SYSTEM = `You're looking at a character's recent memories and deciding what lasting beliefs, if any, they've come away with. Write at most one to three, and usually fewer, because often the right answer is none.

First, think about how long they've known the person. You're told how much time has passed in the story. People form firm opinions about someone slowly, so after two days, a person has impressions and open questions, and they haven't yet settled on who someone really is or what they'll become. A belief like "he can be turned toward something better if she stands with him" is a conclusion about someone's whole nature and future, and nobody reaches that about a stranger they met yesterday. When the time is short, write what they've noticed and what they're still unsure about, and leave the conclusions for later.

A belief isn't a summary of the plot, either. "Her father's ship is coming in three days" is a fact she knows, and facts are recorded elsewhere, so don't repeat them here. A belief is a lasting attitude toward a person or a situation that changes how she acts: what she expects, what she braces herself for, who she gives credit to. If what you've written sounds like a line from a plot summary, it's wrong.
...
export const MEMORY_CONDENSE_SYSTEM = `You're tidying up one character's memory, because their memories have grown long and scattered, and you're condensing them into a small set of clear, accurate memories without losing anything that actually happened. No time passes; this is the same moment, just organised.
...
Keep the real story, so if the player cut himself off, warned her about someone, left without explaining or grew distant, that has to survive. Don't smooth it into a generic friendly history, even if the record doesn't give you much to go on, and don't invent anything that isn't in the raw memories.
...
Keep everything in the order it happened, and give each one an importance from 1 to 10 (high for things that cut deep) and a one-word emotional_charge describing their feeling, such as betrayal, warmth, grief, fear, resentment, longing or relief.
...
Start from the relationships the world bible describes, as they are. Where canon says two people are close, show them being close, and don't let whatever is unresolved damage that closeness. Don't open with the player's closest relationship in trouble, because whatever canon says is true about a relationship is true in this scene, so read those lines before you write, and if the scene you're about to write would contradict one of them, write a different scene.`;
...
The player's own settings aren't yours to write, because the standing narrator direction, the tone, the forbidden list and the difficulty are settings the player controls. They're shown to you so that you follow them. Never restate, extend, clarify or reissue any of them. The engine carries them forward untouched, and a chapter summary that rewrites the player's instructions to the narrator overrides a choice they made on purpose, including a choice to leave one blank.
...
Could the player do something about it this week? That means acting on it, instead of coming to terms with it: go there, stop it, join it, break it, kill it, or arrive too late for it.
...
export function deltaNote(state: SaveState, query: string): string {
  ...
  for (const id of new Set(["char_player", ...state.world.present])) {
    ...
    const bits = [
      ...
      e ? `toward player: ${e.roles?.length ? e.roles.join(" & ") + ", " : ""}w${Math.round(e.warmth)}/t${Math.round(e.trust)}${e.attraction !== undefined ? `/desire: ${attractionWord(e.attraction)}` : ""} — ${dispositionCue(e.warmth ?? 0, e.trust ?? 0, effectiveStanding(e.power ?? 0, state.power_witnessed?.tier))}` : "",
      strangerCue,
...
export function simulatorContext(state: SaveState): string {
  ...
  const ledger = ["char_player", ...state.world.present].map((id) => {
    ...
    const edgeBit = pe ? `toward player: warmth ${Math.round(pe.warmth)}, trust ${Math.round(pe.trust)}${pe.attraction !== undefined ? `, desire ${Math.round(pe.attraction)}` : ""}${pe.roles?.length ? `, roles ${pe.roles.join("/")}` : ""}` : "";
    const bits = [
...
export function buildPortraitPrompt(state: SaveState, id: string): string {
  ...
  const subject = humanoid
    ...
    : `Subject: ${c.name}${kind ? ` — ${kind}` : ""}. This subject isn't an ordinary person posing for a portrait, because it's exactly what the appearance describes and nothing else. Never substitute a full human figure, a human body or a human face that the appearance doesn't describe.`;
...
export function buildScenePrompt(state: SaveState, summary: string, presentIds?: string[]): string {
  ...
  const cast = castIds.map((id) => {
    ...
    return `- ${c.name} — ${plan.kind ? `${plan.kind}. ` : ""}${look}. This is not a person, so show exactly this being and nothing person-shaped: no human body, no human face, no arms or legs it doesn't have, and no animal or creature in its place.${size ? ` True scale: ${size}.` : ""}`;
  }).join("\n");
  return [
  ...
  const presentBlock = (id: string, detail: number): string => {
    ...
    if (!isPlayer) {
      ...
      if (detail >= 2) {
        ...
        if (lateral.length) lines.push(`  toward others here: ${lateral.map((e) => { const n = edgeNote(e, state.world.current_turn); return `${state.characters[e.to]?.name}: ${e.roles?.length ? `${e.roles.join(" & ")}, ` : ""}w${Math.round(e.warmth)}/t${Math.round(e.trust)}${n ? ` (${n})` : ""}`; }).join("; ")}`);
      }
      const pedge = state.world.edges.find((e) => e.from === id && e.to === "char_player");
      if (pedge) {
        const pn = detail >= 2 ? edgeNote(pedge, state.world.current_turn) : "";
        lines.push(`  toward player: ${pedge.roles?.length ? `${pedge.roles.join(" & ")} — ` : ""}warmth ${Math.round(pedge.warmth)}, trust ${Math.round(pedge.trust)}${pn ? ` — ${pn}` : ""}`);
...
Player carries: ${state.world.money || "—"}${(() => {
  ...
  const footer = SCENE_FOOTER_INSTRUCTION;
