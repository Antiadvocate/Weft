...
- WHAT THE PLAYER DID TO SOMEBODY'S BODY IS A CONDITION ON THAT BODY. When an act leaves a person unable to do something they could do a minute ago — held in place, unable to speak, blinded, put to sleep, made to obey — record it with facts[] condition_add on THEM, in plain words that say what no longer works: "frozen in place, cannot move", "cannot speak". Without that row the rest of the engine has no idea it happened, and the next turn will have them walk out of the room and answer a question, because nothing anywhere says they cannot. Record the lifting of it the same way, with condition_remove, the moment the prose lets them go.
...
`
  ...
  const neverEngine = b.forbidden_as_primary?.length
    ? `\nAND IT ACTS THROUGH THE PALETTE, NOT THROUGH THESE: ${b.forbidden_as_primary.join("; ")}. Those may be true in this world and the player may steer into any of them; none of them is ever your reason for a scene. Do not open on one, build the turn around one, or resolve the turn with one.`
    : "";
