...
- THE CAMERA DOES NOT INTERPRET: report what a person in the room could see, never what it meant — no captioning a gesture with its significance, no simile whose second half explains the feeling, no delivery adverbs restating the line. Also banned, every one of them caught in play: reaching into a shared past to explain a look ("the way she'd looked at him when they were younger and he'd said something true by accident"); ruling out readings in order to install one ("not frightened, not grateful, just a woman doing arithmetic on a sum she hadn't expected"); asserting how two people always are ("the way it always was with him"); and the whole accounting metaphor for somebody's private conclusion — sums, ledgers, arithmetic, numbers adding up. A look is described or it is not described.
...
"memories":[{"char_id":"","content":"ONE tight sentence in the THIRD PERSON about this character, past tense — their account of what happened, never a quotation and never a line of dialogue on its own. \"She told Rabi she would not chase him\", not \"I don't want to be that woman on the train.\" A quoted span records the words and loses who said them, which is how a player's own text message ended up filed as another character's memory. Never write a memory in the first person: the same character came out as 'I dared him' on one turn and 'she deflected' on another, and a reader cannot tell whose account it is","importance":4,"emotional_charge":"","scheduled_time":"OPEN LOOP — set whenever this memory leaves something UNFINISHED for that character: a message half-delivered, an answer owed, a meeting agreed, a thing due to arrive. Use the in-world time it comes due (\"Day 5, 09:00\"), or \"unresolved\" when there is no clock. Omit only when nothing is left hanging.","anchor":"short VERBATIM span from action/prose containing any specific detail (name/place/number) this memory records","core":false}],\n"facts_learned":[{"char_id":"who learned it","fact":"durable declarative fact, one tight sentence, specifics copied exactly","quote":"verbatim source words establishing it","corrects":"ONLY when this fact OVERTURNS something they already believed — a few words naming the old belief, e.g. \"her father sent his champion\". Omit otherwise."}],
...
export function simulatorContext(state: SaveState): string {
  ...
  const openProms = (state.world.promises ?? []).filter((p) => p.status === "open");
  if (openProms.length) {
