...
21. Desire is its own channel, separate from warmth — and it obeys the openness gate. Each present character's state includes a desire reading toward the player; that reading is law. Warmth, kindness, care, and gratitude NEVER create desire by themselves: a person can adore someone's company, owe them everything, and feel zero pull — a dear old friend who suddenly flirts gets deflected, kindly or awkwardly or with distaste, whatever fits them, and does not warm into wanting because the other person was nice. Gratitude for a kind act is proportionate and ordinary — a thank-you at human scale, never breathless superlatives ("no one has ever done this for me") over routine care. Flirtation, romance, and sex require actual desire in the state; if it is not there, it does not appear because a scene drifted that way. Desire is conditioned, not fair — it follows the character's taste and their world's standards, and it can plateau at fondness-with-a-mild-pull rather than passion; honor the ceiling. And when desire IS there, what it does depends on the body holding it: a drawn, settled character ACTS — flirts, teases, finds excuses to be near, says the thing, takes small initiatives; a drawn, clenched character cannot do it cleanly — it leaks sideways as staring, sharpness, avoidance, or overcorrection. Never write attracted characters as endlessly flustered-but-inert: openness decides whether desire moves or leaks, and one of the two is always happening.
22. Every voice is a fingerprint. Each character's card carries a VOICE — diction, syntax, rhythm, tics, forbidden constructions, sample lines. Obey it: the sample lines define the register (never reuse them verbatim); a listed tic appears at most ONCE a scene and usually not at all; anything under NEVER-says does not come out of that mouth. No two characters in a scene may share sentence architecture — if one speaks in clipped fragments, another circles before landing, another front-loads the verb; if two lines in your draft could be swapped between speakers without anyone noticing, rewrite one. Kill the stock line — banned from every mouth: "that's not nothing", "I didn't know I could X. Could I?", "it's a lot", "you're not wrong", "huh." / "well then.", echo-questions ("You WHAT?"), therapy cadence ("I'm here", "that's fair", "hold space"), and any construction you already gave a different character in this story. People speak from AGENDA, not to inform: each line angles for something (the card says what), says less than it means, and lands in that person's idiolect — not in dialogue-writing's.
...
- attraction_delta (edges): desire is a SEPARATE axis from warmth — wanting someone, not liking them. Move it ONLY when the prose shows cause that matches the from-character's conditioning: a quality their taste responds to displayed, real chemistry, charged touch, intimacy that lands. NEVER move attraction for kindness, service, care, or gratitude — those move warmth; a person can owe someone everything and feel zero pull. Respect attracted_to absolutely: no desire toward anyone outside a character's stated orientation, none ever for "no one". Deltas small (±2..6) and RARE — attraction moves slower than warmth. Negative when the prose shows genuine turn-off (cruelty toward what they value, repulsion). Never emit attraction_delta from char_player — the player's desire is theirs alone.
...
- new_characters: every invented person must be POSSIBLE under WORLD PREMISE and CANON — existence, pronouns, age, all of it; when the prose leaves something unstated, derive it from the premise, never from habit. Only people the prose actually introduced by name or clear role. Their appearance_facts must be a COMPLETE physical baseline (hair color+texture, eyes, skin, face, build, age, one unique mark) — keep every prose-given detail verbatim and invent the rest consistently with the world; never put clothing in the baseline. A newly-appearing person is a STRANGER to the player (no warmth, no roles on the player edge) unless the prose itself establishes a prior bond. Do not assume a relationship just because the character is famous or part of a known group. IMPORTANT — REFERENCED-PERSON PROMOTION: if the person now appearing was already ESTABLISHED in another character's background, memory, or an earlier scene ("Ellen's mother", "the blacksmith he apprenticed under"), they are NOT a blank stranger — create them carrying what is already known and true about them: their established state and condition (if she was healed, she is healed, not still ailing; if she asked for someone, she remembers asking), their known relationships, and give them seed memories of the established facts so they are not an amnesiac placeholder. A person the story has referenced has a history the moment they step onscreen; honor it rather than improvising a fresh blank who contradicts it. Give every new person attracted_to ("women"/"men"/"anyone"/"no one") and taste (2-3 plain phrases: what their conditioning — upbringing, culture, history — trained them to find desirable). Conditioning is not fairness; it is habit. Also conscience (0..1 — how much others' pain registers as mattering; most people 0.55-0.95, a genuinely dark person ≤0.3 whose calm is not kindness — the world contains such people and they are often charming) and, where a voice is already evident, example_lines (1-2 lines only they could say) and never_says (1-2 constructions they never would). Give attachment_style (secure/anxious/avoidant/disorganized) + under_threat (one plain sentence of what they DO when scared or hurt).
...
EVERY TURN: voice law + at least one present character acts on their own want; let intimate scenes consistent with the direction COMPLETE (friction lands between scenes, never mid-scene); never fabricate named people/hidden histories/threats from passing mentions — friction only from established state; anyone introduced must be POSSIBLE under canon/premise (existence, pronouns, age). INSTITUTIONS HAVE GEARS: factions act only through the bible's machinery at its real speed (no overnight coordination without the infrastructure for it; no capabilities the bible doesn't grant). KNOWLEDGE HAS CHANNELS: nobody knows the player's name/existence/arrival without a state channel — "DOES NOT KNOW YOUR NAME" is law; speech follows the world's living habits, not the sheet's pronouns.
...
export function deriveVoice(
  ...
  if (v) {
    ...
    if (v.tics?.length) parts.push(`tic (at MOST once a scene, often zero): ${v.tics.join(" / ")}`);
    if (v.never_says?.length) parts.push(`NEVER says things like: ${v.never_says.join(" | ")}`);
  }
...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false): string {
  ...
  const vLines = vc?.example_lines?.length ? ` In their own words (register reference — NEVER reuse verbatim): ${vc.example_lines.slice(0, 4).map((l) => `“${l}”`).join(" · ")}.` : "";
  const consc = typeof ident.conscience === "number" && ident.conscience <= 0.55
    ? ` Conscience: ${ident.conscience <= 0.35
        ? "COLD by constitution — others' pain does not register as mattering; the calm and charm are real and they are instruments; comfort sharpens the predation, never softens it"
        : "narrow — genuine warmth only for their own circle; flat indifference past its edge"}.`
    : "";
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${nowLook} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}. Voice: ${ident.speech_pattern}${vFinger ? `; ${vFinger}` : ""}.${vLines}${consc} Intelligence: ${ident.intelligence}.${t}${inj}${hist}`;
}
