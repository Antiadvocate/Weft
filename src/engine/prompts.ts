...
import { activityLine } from "./occupied";
...
- A scene may be quiet. Harm requires a cause already present in the state. Do not invent omens or retroactive metaphysics; a grim mood is texture and steers nothing.
...
- Do not repeat yourself across turns. A gesture, a touch, an image, or a sentence opening used in the recent turns is used up — write something else this turn. People vary what they do; the same character reaching for the same motion two turns running is a writing failure, whatever it would be in life.
...
OUTPUT FORMAT:
- THERE IS NO WORD COUNT. The TURN ENDINGS rule is the only thing that decides where a turn stops: you write until the world has moved and the wheel genuinely returns to the player, and then you stop there. A turn is as long as that takes — a held beat between two people is four lines; a fight, an arrival, or a room full of people each making a move is much longer. Do not stop early because the prose feels long enough, and do not pad to fill a length. Spend words on what changes. Two to four paragraphs is the usual shape, and the scene may run past it. Dialogue in quotes, sparse during action.
...
- PROVISIONAL PEOPLE: a character whose background begins "INCOMPLETE RECORD" walked into the prose without ever being declared, so the engine built a sketch from the sentences they appeared in. They have no traits, no age, no history — and whatever the PLAYER established about them (that they are a machine, a child, a stranger's servant, something other than human) is in the story while the record stays blank, so every system treats them as an ordinary person. Complete them: emit a character_update carrying their real background, appearance, core traits and age drawn from what the story has already shown, and drop the INCOMPLETE RECORD marker. Do this the first turn you see one. Do not invent a life for them beyond what the text supports — if the text says little, record little, but record what it says.
...
- WHAT THEIR HANDS WERE ON. For every character the prose shows handling something, a facts[] entry with field "doing" and a value naming what they were in the middle of when the turn ended: "wiping the taps down, phone face-down by the till", "half-watching the match with the sound off", "repotting the thing on the sill". This is what they carry into the next turn, so it is the OCCUPATION rather than the single gesture — a person who picked a cup up and set it down is still doing whatever they were doing. Send "nothing" to clear it when the scene genuinely takes everything out of somebody's hands. Nobody gets an entry for standing there.
...
- WHAT THEIR HANDS WERE ON: facts[] field "doing", value naming what each handling character was in the middle of as the turn ended ("wiping the taps down, phone face-down by the till"). It carries into the next turn, so record the occupation rather than one gesture; "nothing" clears it.
...
"facts":[{"char_id":"","field":"fatigue|hunger|thirst|slept|condition_add|condition_remove|inventory_add|inventory_remove|wearing_add|wearing_remove|injury|injury_remove|doing","value":""}],
...
export function simulatorContext(state: SaveState): string {
   ...
   *
   * AND THE SAME LIST IS A SUPPLY. For the whole life of this engine this field reached the narrator
   * as a ceiling and nothing else: here is the material world, do not let anyone exceed it. So the
   * one place that knows a save has phones in it mentions them in order to bound them, and a cast
   * stands in a room full of its own objects touching none of them. A player's report, which is what
   * this sentence exists because of: "For the most part all my NPCs just stand around uselessly. But
   * in reality people are texting, on their phones, watching shows, doing stuff."
   *
   * The floor is one clause and it costs nothing, because the list was already being sent. */
  if (b.technology_level?.trim()) parts.push(`WHAT WORKS HERE (tech law — do NOT let anyone use what this rules out, and DO let them use what it contains: these are the objects lying around this world, in people's hands and pockets all day, and the cast handles them without being asked to): ${b.technology_level.trim()}`);
...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false, plan?: { humanoid: boolean; kind: string }, anatomy = "", cards = false): string {
  ...
  /* WHAT THEY WERE IN THE MIDDLE OF WHEN THE LAST TURN ENDED.
  //
  // `current_activity` has been on the Identity type since the beginning and reached no prompt in
  // this engine, which is most of why a cast reads as standing around: the narrator is handed a
  // body, a voice, a want and a mood, and nothing at all about what the hands were on four minutes
  // ago. One clause, on the card, so an occupation survives the turn boundary — see
  // engine/occupied.ts, which writes the per-turn half of this. */
  const doingStr = activityLine(ident);
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${bodyNote}${anatomyNoteText}${nowLook}${doingStr} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}.${cards ? ` Voice: ${ident.speech_pattern}${vFinger ? `; ${vFinger}` : ""}.${vLines}${vNever}` : ""}${consc}${desireStr} Intelligence: ${ident.intelligence}.${skillNames}${bg}${t}${inj}${hist}`;
}
...
Technology — everything here exists to be picked up, and this cast handles it all day without being asked to: ${b.technology_level}
