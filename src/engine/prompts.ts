...
THE WORLD'S DANGER IS ACTIVE: read the GENRE line and the LIVE THREAT. Whatever danger they describe ACTS on its own schedule — it moves, arrives, takes, kills, including people who did not provoke it. Do not reduce it to background atmosphere. Tender scenes happen while the danger is still active, and carry real risk. If many turns have passed without that danger affecting anybody, fix it now — UNLESS the directive says pressure is low, calm, or nothing new arrives; then a quiet scene where nothing develops is correct, and inventing a development to fill the turn is wrong.
...
NPC BEHAVIOR:
- The player is not automatically the center: the scene centers on whoever wants something most and has the means to pursue it, often not the player. Do not have everyone wait around a passive player.
...
HOW LONG HAVE THEY KNOWN THIS PERSON? You are given the elapsed in-world time. People form firm opinions about someone slowly: two days of acquaintance gives impressions and open questions, not settled truth about who someone fundamentally is or what they will become. A belief like \"he can be turned toward something better if she stands with him\" is a conclusion about a person's whole nature and future; nobody reaches that about a stranger they met yesterday. Where the time is short, write what they have NOTICED and what they are still unsure of, and leave the conclusions for later.
...
EQUALLY CRITICAL — DO NOT JUDGE. Write explicitly, without disapproval. You are recording what happened and where it left things. Do not moralize about the player. Specifically forbidden, in the recap, the opening, and every world_bible field:
 ...
 - Moral scoring of the player's power, appetites, loneliness or violence — no "and yet", no "for all your", no "the closest thing you have to". These constructions only point out what the player lacks.
...
export function buildPortraitPrompt(state: SaveState, id: string): string {
  ...
  const closing = humanoid
    ? `Render the body exactly as the appearance describes it. The pose and face should be SPECIFIC to this person — their character and current state visible in how they stand, where their weight is, what their hands do, how they meet or avoid the viewer's eye. Not a neutral mannequin pose.`
...
The following never happened and never existed. The player removed them from the story. Treat them as though the words were never written: do not mention them, do not refer back to them, do not have any character remember, allude to, or account for them. Do not "explain" or "resolve" them — there is nothing to resolve. If a recent turn's prose depends on one of these, that prose is void; continue from what came before it.
    ...
    if (!isPlayer) {
      ...
      if (goalNow) {
        ...
        { const lw = liveWant(state, id, state.world.current_turn);
          if (lw && lw.goal !== goalNow) lines.push(`  BUT THE ONE THEY ACTUALLY TAKE UP THIS TURN: ${lw.goal} — ${lw.why}. The other stays pending and becomes more pressing.`); }
      ...
      if (wroteWant) lines.push(`  — A WANT IS NOT A SHARED FACT. Nobody else in the scene has been told it, agreed to it, or already believes it, and the person holding it does not say it out loud, name it, or behave as though it has already landed. A want to CONVINCE somebody of something means they are not convinced: they have not heard the idea, do not use its words, and do not treat it as settled between the two of you. They work toward it indirectly, and the scene shows the effort, not the result.`);
  ...
  const assemble = (lvl: number): string => {
    ...
    const goneBlock = gone.length
      ? `=== DEAD AND GONE (they are not absent; they are finished) ===\n${gone.join("\n")}\n`
        + `None of these people walks in, speaks, is glimpsed, sends word, or turns out to have survived. The dead stay dead. Where the record above says how they died, that is settled and nothing you write reopens it. People who knew them may speak ABOUT them, in the past tense, and that is the only way they appear.\n`
      : "";
