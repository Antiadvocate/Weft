...
- NARRATION AND DIALOGUE ARE BUILT SEPARATELY. The style rules in this section govern the narration only; a spoken line is built by the dialogue procedure below, out of the situation the speaker is in.
...
- THE DIALOGUE is built per the procedure below, out of what is printed under that person's name and the state they are in this minute. Nobody carries a fixed manner of speaking, a signature phrasing, a verbal habit or a catchphrase: what a person says and how much of it comes from what they want in the next minute, what they know, what their body is doing, and who can hear — and it changes when those change. One person builds differently-shaped sentences tired than rested, and that is the only thing that shapes them.
...
- NOBODY HAS A VOICE OF THEIR OWN TO WRITE TOWARD. Do not give anyone a manner of speaking, a signature rhythm, a recurring phrase, a verbal habit, a way of trailing off, a characteristic sentence length, or anything else a reader could use to name the speaker with the attribution removed. What separates one person's line from another's is what they want, what they know, and what has just happened to them — never how they talk. Write everyone plainly, in the same plain register, and let the difference be in the substance.
...
- YOU ARE GIVEN NO SAMPLE OF ANYBODY'S SPEECH AND YOU ARE NOT OWED ONE. There is no stored account of how any of these people talk, because they do not have one. Build every line from the block under that speaker's name — what they want, what they know, their body, who can hear — and from nothing else. Somebody exhausted, drunk, or terrified repeats themselves, stops halfway, and says less; that is the state talking, not a manner they own.
...
STYLE: this document's compressed style never appears in narration. NARRATION AND DIALOGUE ARE BUILT SEPARATELY — these style rules govern the narration only. NARRATION stays the same throughout, built from (1) the direction's style note, then (2) the words this world actually contains — only name objects, materials, jobs, foods, animals, distances and units that exist here. Never name a genre, author or franchise, and do not write toward how books of a kind are usually written. Do not change how you write because the player's last message was written a particular way. DIALOGUE: build every spoken line from its own speaker's card and their state this minute. Nobody has a manner of speaking, a signature rhythm, a recurring phrase or a verbal habit — everyone is written plainly, in the same plain register, and what separates one line from another is what the speaker wants, knows and has just had happen to them. Keep the established person and tense; never address the reader as "you"; engine terms (state, clock, thread, ledger, direction) never appear in prose; never contradict your own prior narration.
...
- UNEARNED APTITUDE, naming the skill: "Could untangle any knot before she could read; still does it while thinking." "Can tell what a horse is going to do a beat before it does it, and never learned how."
...
voices: nobody gets one. Do not record a manner of speaking, a signature rhythm, a recurring phrase, a verbal habit or a sample line for anybody — no character in this world has a speech signature, and the narrator is given none to write toward. What a person has words for comes from their background and their skills, which you are already writing.
...
/** HOW THIS PERSON IS SPEAKING RIGHT NOW — and only right now.
 *
 *  Nobody in this engine has a voice of their own. There is no stored manner of speaking, no
 *  signature rhythm, no tic and no sample line, because a fixed speech signature is the thing that
 *  turned every character into a caricature of one sentence written at creation and repeated for
 *  the rest of the story. What is left is situational and nothing else: what play has made of
 *  them, the pressure they are under this minute, and who they happen to be talking to. All of it
 *  moves turn to turn, which is the point — nothing here is a property of the person.
 *
 *  Empty is a valid answer. A person under no pressure, addressing nobody in particular, has
 *  nothing to say about how they are speaking, and the caller omits the line entirely. */
export function deriveDelivery(
  ...
  const parts: string[] = [];
  // strong acquired traits are what play has made of them (intensity ≥ 5), strongest first
  const strong = [...traits].filter((t) => t.intensity >= 5).sort((a, b) => b.intensity - a.intensity).slice(0, 2);
  for (const t of strong) parts.push(`what they say now carries: ${t.label}`);
  ...
  if (addresseeEdge) {
    ...
    if (warmth >= 40) parts.push("to THIS person: warm, familiar, less ceremony");
...
export function charCard(id: string, ident: Identity, cond: Condition, traits: { label: string; intensity: number; behavioral_impact: string }[], stable = false, plan?: { humanoid: boolean; kind: string }): string {
  ...
  return `${ident.name} [${id}] — ${ident.pronouns ? `${ident.pronouns}, ` : ""}${ident.age},${body} ${ident.appearance_facts} (constant).${bodyNote}${nowLook} Core: ${ident.core_traits.join(", ")}. Values: ${ident.values.join(", ")}.${consc}${desireStr} Intelligence: ${ident.intelligence}.${skillNames}${bg}${t}${inj}${hist}`;
}
...
export function volatileDigest(state: SaveState, query: string, opts?: { budgetOverride?: number }): string {
  ...
  const presentBlock = (id: string, detail: number): string => {
    ...
    if (!isPlayer) {
      ...
      const pedgeForDelivery = state.world.edges.find((e) => e.from === id && e.to === "char_player");
      // SITUATIONAL ONLY, AND OFTEN EMPTY. There is no stored voice to fall back on: when nothing
      // is pressing on this person and they hold nothing toward the player, the line is omitted
      // rather than padded with a manner they do not have.
      const delivery = deriveDelivery(ident, cond, isPlayer ? [] : traits, pedgeForDelivery);
      if (delivery) lines.push(`  speaking right now: ${delivery}`);
