...
WRITING A LINE. Five fields decide it, and all five are printed under that speaker's name — in PRESENT — LIVE STATE, and again in HOW THESE PEOPLE SPEAK at the end of your context, which is the copy to read because it is the nearest one. Read them in this order; the line is what comes out of the answers.
...
- WHAT IS PRINTED UNDER EACH NAME IN "HOW THESE PEOPLE SPEAK" IS WHERE EVERY LINE COMES FROM. You are given no sample of anybody's speech — deliberately, because a sample written to demonstrate a voice is always a short one, and a cast built from short samples all talk in clipped fragments. What you are given instead is the thing samples were standing in for: how old this person is and what a mouth that age can build, how their sentences are put together, how their talking moves, what their life gave them words for, what has happened to them lately, and what they hold to be true. Build the line out of those. A listed verbal habit appears at most once per scene. NEVER-says entries are never spoken by that character. What is printed about how this person talks holds in charged moments too — a character described as never pressuring anyone does not start pressuring because the scene is tense. A state on their block overrides it in one direction only: somebody exhausted, drunk, or terrified repeats themselves, stops halfway, and says less than they otherwise would.
...
/** AGE IS A LANGUAGE CAPACITY, NOT A FLAVOUR.
 *
 *  This function used to hand everyone from a toddler to a twelve-year-old the same four words —
 *  "a child's plain, direct cadence" — and that was the ONLY thing anywhere near the point of
 *  generation that said anything about a character's age at all. "Plain and direct" is a register.
 *  It is not a limit, and a narrator handed a register with no limit writes a competent adult
 *  argument in short words. Measured on the save that produced the complaint, a THREE-YEAR-OLD:
 *
 *      "I eat dinner before sweets," she said. "I ate the eggs. They were on the plate."
 *      She held up the last wedge of honey bread as evidence.
 *      "And I wash my hands. You saw me, at the basin. There was water."
 *
 *  That is a rebuttal. It reads the accusation, concedes nothing, marshals three pieces of
 *  supporting evidence in support of a general claim, and produces a physical exhibit. Elsewhere in
 *  the same save she wins arguments by distinguishing cases ("I won't be too big. I'll fit
 *  sideways.") and by correcting a premise ("Cats don't have wings. They have feet."). Every one of
 *  those is plain and direct, so every one passed. The player's word for it was that his
 *  three-year-old was talking like a fifteen-year-old with an attitude, and he was right.
 *
 *  Her own card knew better — "simple counting to three", "fast little clumps, no pause for reply" —
 *  but the card lives in the cached prefix thousands of tokens up, and position wins.
 *
 *  So the bands below the teens are split by what a mouth that age can actually build, stated as
 *  structure rather than tone. The bands above are unchanged. */
function ageBand(age: number): string {
  if (age <= 2) return "barely verbal — single words and two-word pairs, naming what is in front of them and asking for it; no sentences";
  if (age <= 4) return "a very small child's speech: three to six words at a time, ONE clause, present tense, no subordinate clauses and no connectives beyond 'and'. They name a thing, want a thing, or ask; they repeat themselves and they interrupt themselves. They do NOT build an argument, hold a position across sentences, marshal evidence for a claim, concede a point, correct somebody's premise, distinguish one case from another, use irony, or track what the other person is trying to establish — those are the moves of a much older mouth in short words, and they are the way this goes wrong. A refusal at this age is 'no' or 'want it', repeated, not a reason";
  if (age <= 7) return "a young child's speech: one main clause at a time, ordinary words only, reasons offered as bare 'because' with no argument built on top; they announce, ask, and interrupt, and they lose the thread of what was being discussed";
  if (age <= 12) return "a child's plain, direct cadence — full sentences, concrete subjects, and reasoning that runs one step and stops";
  // "a teenager's slangy, testing cadence" was the whole of this band, and both halves were wrong.
  // "Slangy" asserts a modern register over every world the engine can build — a nineteen-year-old
  // running a Roman household since she was ten does not have slang, she has Latin — and the world's
  // own vocabulary is already read from the bible two lines above this block. "Testing" is a
  // quality-noun with no operation in it: it resolves, in practice, to attitude, which is the exact
  // register the complaint that produced these bands was about. What is actually true of a mouth
  // this age is structural, so that is what it says.
  if (age <= 19) return "full adult grammar, and a shorter stock of things to compare anything to: they state a case in absolutes, take an exception personally, and have not yet learned to leave the last word alone. They can build the argument; they have not yet learned when not to";
...
export function deriveVoice(
  ...
  // The age band used to be pushed here too. It is now printed beside each name in HOW THESE PEOPLE
  // SPEAK, which sits at the end of the digest immediately before the model writes; the bands got
  // long enough (a toddler's needs a paragraph — see ageBand) that carrying them in both places
  // pays twice for the same sentence, and this is the copy that loses on position.
...
${recentStr}${proseTail}${(() => {
  ...
  const lines = state.world.present
    ...
    .map((id) => {
      ...
      // AGE, WITH WHAT AGE MEANS. The header of this block says "AGE IS NOT DECORATION" and then
      // printed a bare number, which is decoration. What a mouth that age can actually build is the
      // part the narrator needs, and it is the part it was inventing — see ageBand.
      const band = typeof c.age === "number" ? ageBand(c.age) : "";
      const out = [`${c.name}${c.age ? `, ${c.age}${band ? ` — ${band}` : ""}` : ""}`];
      // HOW THEIR SENTENCES ARE BUILT.
      //
      // This block ends by telling the model that how much somebody says is a property of the person
      // at this moment, and that a cast where everyone is brief is a cast with one person in it. It
      // then supplied nothing at all to build a long or a short or a run-on line out of. The fields
      // that answer it — voice.syntax ("stacked questions with no answer between them"), voice.rhythm
      // ("breathless run-ons under threat, then a sudden count on her fingers to steady") — existed
      // on every character, and were printed ONLY on the cached identity card thousands of tokens up,
      // deliberately, to avoid paying for them twice.
      //
      // That trade was wrong, and the save that showed it had a woman whose entire recorded voice is
      // panic and stacked questions saying "I'll come." and "It won't get wet." for forty turns. The
      // duplication this avoided is real; what it bought was a cast with no syntax anywhere near the
      // point of generation, which is where syntax is decided. So the STRUCTURAL half comes down here
      // — how the sentences are built and how the talking moves. Diction stays on the card: it is
      // about vocabulary, which survives distance because it is a word list, and repeating it is the
      // duplication that was worth removing.
      const shape = [c.voice?.syntax, c.voice?.rhythm].map((x) => String(x ?? "").trim()).filter(Boolean).join("; ");
      if (shape) out.push(`   how their sentences are built (this decides LENGTH and SHAPE, and it is not optional): ${shape}`);
      // The tic is the first thing to go under budget: it decorates a line, where `shape` decides
      // whether the line exists at the right length at all.
      if (lvl >= 2 && c.voice?.tics?.length) out.push(`   verbal habit, at most once in the scene: ${c.voice.tics.slice(0, 2).join(" / ")}`);
...
EVERYONE LISTED HERE IS IN THE SAME ROOM AND CAN HEAR EACH OTHER, AND PEOPLE IN A ROOM TOGETHER TALK. Some of this turn's words are theirs, in quotes: a turn of bodies, weather and posture with nobody speaking is a turn nobody was in. BUILD EACH LINE OUT OF WHAT IS PRINTED UNDER THAT SPEAKER — their age and what a mouth that age can build, how their sentences are put together, how their talking moves, the traits under "as:", the life behind the words, what happened to them lately, what they hold to be true, the state they are in. You get no sample of anyone's speech: a sample is always short, and a cast built from short samples all talk in clipped fragments. What a sample stood in for is printed above instead.

HOW MUCH SOMEBODY SAYS IS NOT A PROPERTY OF THE PERSON. It is a property of the person at this moment. The same woman refuses in four words and then spends ninety explaining how the levy is worked out, because the explaining is the part she likes. Decide the length of every line from what that speaker wants out of the next minute and how much the listener already knows — never from a house style, and never from how the last person spoke. A CAST WHERE EVERYONE IS BRIEF IS A CAST WITH ONE PERSON IN IT: somebody here talks too much, somebody answers a question that was not asked, somebody explains at length to a person who did not want to know.
AGE IS NOT DECORATION — the band printed beside each name is what that mouth can actually build, and it binds. The number is the current record: if anything else implies a different one, that is stale and this is right, and nobody says the stale number out loud.
