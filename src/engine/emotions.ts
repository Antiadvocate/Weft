...
/**
 * Emotions â€” the lifecycle of feeling, and how bodies regulate each other. Deterministic, zero tokens.
 *
 ...
 *    - SETTLED body (relaxation high): the state dissolves on its own after a couple of turns â€”
 ...
 *      starts feeding on itself â€” a small relaxation drain each turn, because the reaction to the
 ...
 *    present in the room, pulls you toward settled â€” that is what a safe person is FOR. But HOW a
 ...
 *    - anxious: runs HOT toward people â€” soothed strongly by presence, but when scared with no safe
 *      person in the room, the alarm feeds itself (protest: escalate, cling, re-check).
 *    - avoidant: runs COLD â€” comfort barely lands, and when actually threatened, closeness itself
 *      is pressure: a warm person leaning in does nothing (they settle alone, later).
 *    - disorganized: wants the comfort and fears it in the same motion â€” presence helps some turns
...
/** Structural states other systems own â€” the lifecycle leaves them alone. */
const STRUCTURAL = [/^fixated on /i];

/** What a released emotion leaves behind â€” the feeling's information, kept after the charge goes. */
...
function residueFor(stateName: string): string {
  ...
  return "passes on its own â€” felt fully, not fed";
}
...
export function tickEmotions(state: SaveState): string[] {
  ...
  for (const id of state.world.present) {
    ...
    for (const st of emotional) {
      ...
      if (r >= 3 && age >= 2) {
        // SELF-LIBERATION: nothing is gripping it, so it goes â€” and leaves its information.
        ...
        shifts.push(`${c.name} keeps re-telling the ${st} â€” the reaction has become its own pain now.`);
      }
    }
    ...
    if (p.mood && p.mood !== "even" && r >= 2 && p.mood_set_turn !== undefined && turn - p.mood_set_turn >= 4) {
      shifts.push(`${c.name}'s ${p.mood} passed â€” weather, not climate.`);
      p.mood = "even";
...
function coRegFactor(style: string | undefined, threatened: boolean, turn: number, id: string): number {
  switch (style) {
    ...
    case "disorganized": {
      ...
      return ((turn + Math.abs(h)) % 3 === 0) ? -0.4 : 0.8;  // helps most turns, stings some â€” comfort and threat in the same motion
    }
