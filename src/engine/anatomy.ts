...
export function anatomyNote(rec: AnatomyRecord | null, name: string, pronouns?: string): string {
  ...
  return ` ANATOMY (BINDING, ESPECIALLY WHAT IT DOES NOT HAVE): the record gives ${name} a ${hasList}, and gives ${name} nothing else of the kind: ${DENIAL[rec.lacks[0]](poss, obj)}. This is the body this person has, it is the body the player wrote, and it does not change to suit a scene. Where your training would supply the other configuration because of what ${name} is called, what pronouns ${name} uses, how ${subj} is dressed, or what is being done to ${obj}, that default is WRONG HERE and you must catch it before you write it. Every act, every touch, every description and every line of dialogue about this body uses only what is named above. If a moment cannot be written without the part ${name} does not have, write a different moment.${rec.evidence ? ` The record's own words: "${rec.evidence}"` : ""}`;
}
