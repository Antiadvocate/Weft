...
export function anatomyNote(rec: AnatomyRecord | null, name: string, pronouns?: string): string {
  ...
  return ` ANATOMY (this has to be followed, especially what they don't have): the record gives ${name} a ${hasList}, and gives ${name} nothing else of that kind: ${DENIAL[rec.lacks[0]](poss, obj)}. This is the body this person has and the body the player wrote, and it doesn't change to suit a scene. If your habits would fill in the other arrangement because of ${name}'s name, pronouns or clothes, or because of what's being done to ${obj}, that assumption is wrong here, and you have to catch it before you write it. Every act, touch, description and line of dialogue about this body uses only what's named above. If a moment can't be written without the part ${name} doesn't have, write a different moment.${rec.evidence ? ` The record's own words: "${rec.evidence}"` : ""}`;
}
...
export function anatomyFix(hit: AnatomyHit | null | undefined): string {
  ...
  return `\nLAST TURN THE PROSE GAVE ${hit.name.toUpperCase()} A BODY THE RECORD DOESN'T GIVE ${obj.toUpperCase()}: "${hit.sentence}…"
${hit.name} has no ${hit.part === "vulva" ? "vagina, vulva, labia or clitoris" : "penis, cock or testicles"}, and never did. That sentence doesn't count: it didn't happen, nobody remembers it, and nobody mentions it again. Don't correct it inside the story, don't have anyone comment on it, and don't write a scene explaining it. Carry on as though the body had been written correctly all along. This turn and every turn after it, ${hit.name}'s body is only what the record names, in the narration and in every character's mouth, whatever kind of language the scene uses, because the clinical word, the affectionate word and the crude word are all ruled out in the same way. If the scene reached for that part because of ${hit.name}'s name or how ${subj} is dressed, that's the assumption to avoid.`;
}
