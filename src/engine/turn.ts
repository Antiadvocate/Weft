...
function deriveDefaultValues(traits: string[], background: string): string[] {
  ...
  while (out.length < 2) out.push(out.length === 0 ? "being treated with respect" : "something that is theirs alone");
...
PUTTING THINGS NEXT TO EACH OTHER: when you show something observable and a conclusion could be drawn from it, write the observable thing and end the sentence there. Don't link it to a conclusion with a word like "saw that" or "because". Let the next sentence be the next thing that happens, instead of a comment on the one before.`;
  ...
  // with a run of them still produces one correction rather than a lecture.
  {
    ...
    if (state.last_risen) {
      ...
      ev.onMeta({ shifts: [`${r.name} is ${r.status} — that part was discarded from the record`] });
    ...
    try {
      ...
      if (beats.trim()) {
        ...
        const contract = [
          ...
          bibleC.forbidden_as_primary?.length ? `NEVER WHAT DRIVES THE STORY: ${bibleC.forbidden_as_primary.join("; ")}` : "",
        ].filter(Boolean).join("\n");
...
function unregisteredSpeakers(state: SaveState, prose: string, action = ""): string[] {
  ...
  for (const pu of diff.places_update ?? []) {
    ...
    shifts.push(`${place.name} has changed${pu.note?.trim() ? ` — ${pu.note.trim()}` : ""}.`);
  ...
  for (const c of diff.consequences_new ?? []) {
    ...
    shifts.push(fire_time ? `A consequence is scheduled for ${fire_time.replace(/\s*\(.*\)$/, "")}.` : `A consequence was scheduled for a coming turn.`);
  }
