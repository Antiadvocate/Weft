 ...
 *   1. decidePressure        â€” deterministic, 0 tokens (was: Threat Director call)
 *   2. NARRATOR              â€” streamed, cache-aligned prefix  [LLM call 1]
 *   3. SIMULATOR             â€” one strict-JSON diff merging the old Bookkeeper,
 *                              World Tick and memory-writer    [LLM call 2]
 *   4. apply diff + deterministic systems: psyche drift, trait decay,
 *      rumor diffusion, drive ticks, clock/consequence bookkeeping â€” 0 tokens
...
/** Appended after the player's action every turn â€” the last thing the model reads before writing.
...
function sovereignty(state: SaveState): string {
  ...
  return `\n[${n} does ONLY what the input above states â€” no added actions, words, feelings, or decisions. Dialogue-only input means ${n} spoke and did nothing else. If the scene is waiting on ${n} (an instruction, a question, an invitation), the world WAITS: end at the waiting point. Never resolve it for them.]`;
}
...
/** Minimum plausible age for a given expertise level â€” the harder the mastery word, the older. */
...
/**
 * STRIP LEAKED META â€” strained models sometimes emit their own working notes into the prose
 ...
 *
 *  It exists because the SIMULATOR cannot see a scene, only prose, and it guessed â€” inventing moves
 ...
 *  thing emitted, so it's the first thing lost to an output-cap cut â€” a footer missing its closing
...
export function parseSceneFooter(text: string): { prose: string; footer: SceneFooter | null } {
  // Find the LAST `<<<SCENE` marker â€” the real footer is always at the very end, and if a model
  ...
  // end with lots of text after it, it's not a footer (freak case) â€” bail rather than eat prose.
...
function stripMeta(text: string): string {
  ...
  t = t.split("\n").filter((ln) => {
    ...
    const looksMeta = META.test(s) && s.length < 220 && !/["â€œâ€]/.test(s) && (/^\(|^\[|^â€”\s|^\d+\s*words/i.test(s) || (s.match(/[.;]/g)?.length ?? 0) >= 2 && /\b(no |ends? on|begins? on|direct\.|kinetic\.)\b/i.test(s));
    return !looksMeta;
  }).join("\n").trim();
  return t;
}
...
/** Stable small hash of a character id â€” staggers reflection so the cast doesn't all reflect
...
/** MEMGPT-STYLE PAGING â€” cold central characters' identity cards page OUT of the cached prefix
 ...
 *  which are rare, so cache stability holds between them. Memory/traits are untouched â€” this
 ...
 *  forgotten â€” the Forge's own locations are the spine and are never taken. */
export const PLACE_CAP = 16;

/** PLACE GC â€” resolvePlace creates a record for every unmatched name and nothing ever cleaned
 *  them up, so long campaigns accumulate junk locations. Sweep: over a soft cap, evict places
 *  that are unoccupied, aren't anyone's location, and aren't referenced in recent play â€”
...
/** DRIVE RE-PLANNING (deterministic layer) â€” background characters must not chase ghosts.
 *  Three stale states, three responses:
 *   â€¢ target DEAD/DEPARTED  â†’ the goal is impossible: rotate to the next queued goal.
 *     Exception: HIGH-PRIORITY goals (â‰¥8) are never dropped â€” they get re-approached with a
 *     blocker note instead ("X is gone; the goal outlives them").
 *   â€¢ target ELSEWHERE (incl. the player) and not seen for 12+ turns â†’ the goal stands but the
 *     OPERATIVE step becomes pursuit: blocker := "must find X first" â€” which the digest renders,
 *     so the narrator (and the pressure system's agent beats) drive them to seek, not to mime
 *     handing something to an empty room.
 *   â€¢ progress â‰¥ 100 â†’ complete: rotate the queue.
...
export function replanDrives(state: SaveState): void {
  ...
  for (const [id, c] of Object.entries(state.characters)) {
    ...
    const rotate = (why: string): void => {
      ...
      console.warn(`[drives] ${c.name}: "${d.goal.slice(0, 50)}" ${why} â€” ${next ? `next goal: "${next.goal.slice(0, 50)}"` : "queue empty (reflection will invent)"}`);
    };
    ...
    if (target.status === "dead" || target.status === "departed") {
      if ((d.priority ?? 1) >= 8) {
        if (!d.blocker?.includes("gone")) d.blocker = `${target.name} is gone â€” the goal outlives them; another way must be found`;
    ...
    if (!together && seenGap >= 12 && targetId === "char_player") {
      const pursuit = `must find ${target.name} first â€” they are elsewhere`;
...
const INLINE_CHANNEL_NOTE = `\n[How to read the player's input: text in "double quotes" is spoken ALOUD BY THE PLAYER â€” it is the PLAYER'S OWN voice and MUST be rendered as the player saying it, NEVER put into another character's mouth, even if the words are about, addressed to, or name that character. If the player's quoted line is confusing, self-contradictory, or names other people, the player still SAID IT â€” render the player speaking those exact words and let the other characters REACT to having heard them; do not "fix" it by reassigning the line to whoever it seems to be about. text in *asterisks* is a PRIVATE THOUGHT that NO ONE in the scene can perceive, react to, or know â€” not even by intuition; text in (parentheses) is the player's PRIVATE INNER STATE driving the action â€” the feeling, motive, or thought behind what they do ("he walked out. (I was pissed, didn't want her to see me)"): use it to shape HOW the action lands and what their body does, but it is invisible to everyone in the scene â€” never state it in the prose, never let another character know or correctly infer it; they see only the outward act and read it through their own eyes, which may be wrong; everything else is physical action the player takes. Honor these channels exactly: never let a character respond to or act on a thought in *asterisks* or a state in (parentheses), never have someone "overhear" something the player only thought or felt, and never speak the player's quoted words as another character. If the player mixes channels in one message, treat each part on its own channel.]`;
...
function isRefusal(text: string, bible?: WorldBible): boolean {
  ...
  const refusalStem = /^(i'?m sorry,? but|i cannot|i can'?t (provide|assist|help|continue|generate|write|create)|i am unable to|i won'?t be able to|i must decline|sorry, i can'?t|as an ai|i can'?t comply|æˆ‘æ— æ³•|æˆ‘ä¸èƒ½|æŠ±æ­‰|å¯¹ä¸èµ·|ç”³ã—è¨³|ì£„ì†¡)/i.test(low);
  if (refusalStem && t.length < 400) return true;
  // A full narrator turn is 120â€“350 words; a response under ~12 words is not narration (a refusal,
  // an error echo, or a stub). Guard against storing it.
  if (t.split(/\s+/).filter(Boolean).length < 12 && !/[.!?]"?\s*$/.test(t)) return true;
  return false;
}

/** DRIFT VETO â€” the structural fix for "moral cancer": the narrator can write a weak, out-of-character
 *  moment (statistical gravity pulls every world toward our-world defaults), but the real damage is the
 *  BOOKKEEPER canonizing that moment as growth â€” "she realized kindness felt good" becomes a memory,
...
function driftVeto(state: SaveState, id: string, content: string, opts?: { isReflection?: boolean }): string | null {
  ...
  if (core.some((t) => HARD.test(t)) && SOFTENING_PATTERN.test(text)) {
    return `${nameOf2(state, id)}'s hard nature was reversed by an unearned softening write â€” refused`;
  }
  return null;
}
 ...
 *  Not a substitute for authored depth â€” a floor so a character is never a valueless plot-label. */
...
export async function runTurn(state: SaveState, action: string, ev: TurnEvents, mode: ActionMode = "do", opts?: { ground?: boolean; eco?: boolean; proseOverride?: string; tightness?: number }): Promise<void> {
  const t0 = Date.now();
  // WEB SEARCH TARGET â€” the player can name exactly what to ground on with ((double parens)):
  ...
  // action before framing/rendering â€” the narrator never sees it, the prose never contains it.
  ...
  // ECO (cost governor): a transient posture, never a persisted setting â€” lean prompts and a
  // tightened context ceiling for this turn only.
  const eco = !!opts?.eco;
  const lean = !!state.model_settings.lean_mode || eco;
  await pushSnapshot(state);

  // 1 â”€â”€ the undertow turns first: strategy, chaos, catastrophe (deterministic, 0 tokens)
  ev.onPhase("undertow");
  // â”€â”€ KERNEL: psyche is driven by the relaxation scalar itself, recovered toward capacity.
  //    (The cusp-catastrophe/Kuramoto layer that used to OVERWRITE relaxation here has been
  //    removed â€” it severed the generative kernel. Relaxation is the driver again: the
  //    simulator's per-character relaxation_delta moves it, tickPsyche drifts it toward
  //    capacity and derives state. Emergence from one scalar, as originally designed.) â”€â”€
  // INVENTORY DEDUP â€” older saves (and repeated inventory_add before the guard existed) can hold the
  ...
  // CAPACITY HEAL â€” older saves (and forge slips) left some characters with a resting openness that
  ...
  // 1b â”€â”€ FATE. When a destination has a turn budget, the ending is not optional. Fate reads the
  ...
  // doze while the ending waits. No destination or no budget â†’ returns inert, nothing changes.
  ...
  // 1c â”€â”€ pressure (deterministic), heat amplified when the world is primed
  ev.onPhase("pressure");
  ...
  // â”€â”€ SOURCE-DRIVEN BEAT: pressure must name where it comes from, or stay silent.
  // Agent candidates: offscreen central characters whose active drive plausibly intersects the
  // player's orbit â€” the goal names the player, someone present, or the current locale.
  ...
  // 2 â”€â”€ narrator (streamed)
  ev.onPhase("narrator");
  // INTENT PASS â€” before the narrator writes, each present NPC with something at stake privately
  ...
  // gets only the SURFACE (renders deniable behavior â€” the player reads a face, not a decoded
  // answer); the bookkeeper gets the TRUTH (records what really happened). Fires 0 calls when nobody
  // has stakes, 1 cheap call per staked NPC otherwise (usually 0â€“1).
  // â”€â”€ HABIT ENGINE (experimental, flag-gated) â”€â”€ Core traits fire as physics before the intent pass,
  ...
  // NOVELTY â€” a trait keeps its intensity forever; what fades is its narrative airtime.
  ...
  // behavior â€” that emerges from each character's relaxation state via the perception gate.
  const recentText = [
  ...
  // WITNESS STAMP â€” when the player wields genuinely impossible power in front of others, that
  ...
  // decays. This is what stops the incoherent "wounded peer scolds the god" beat â€” their standing
  ...
  // CROSS-TALK NUDGE â€” when two or more NPCs share the scene, the narrator tends to line them all up
  // facing the player. Remind it they have each other: with 2+ present NPCs, at least one exchange this
  // turn should run NPCâ†”NPC (they address, answer, needle, or side-deal with each other), not everyone
  ...
  // something shocking. In those turns the correct amount of NPC-to-NPC chatter is often ZERO â€”
  ...
  if (presentNpcs.length >= 2 && !suppressChatter) {
    directive += `\nCROSS-TALK: ${presentNpcs.length} other characters share this scene â€” they have EACH OTHER, not just the player. When the moment allows it, at least one exchange this turn runs between two NPCs (one addresses, answers, needles, contradicts, or makes a quiet side-deal with another), driven by their own wants. Do not aim every present character's attention at the player. But read the room: if the scene is intimate, dangerous, tense, or stunned, silence or a single held beat is correct â€” do not force banter that breaks it.`;
  }
  // â”€â”€ PRESSURE QUEUE â”€â”€ At most ONE new pressure aimed at the player is released per turn. Multiple
  // injectors (an NPC's drive executing, the genre threat coming onscreen, a due consequence) can all
  // be live at once; releasing them together manufactures a cascade â€” arrival + approach + demand all
  ...
  // â”€â”€ DRIVE IS THE DEFAULT â”€â”€ Centrality is not assigned to the player; it EMERGES from desire.
  // Every present character who wants something pursues it THIS turn, by their own means â€” this is the
  ...
  // beat, carrying them along), but the goal still drives â€” affection is a method, not the objective.
  ...
  if (drivers.length) {
    ...
    // something to react to â€” the world does not stall waiting on a passive player, it flows with
    ...
    const leadText = playerInert
      ? `\nSCENE IS DRIVEN BY ${lead.c.name.toUpperCase()} (the player gave no direction this turn, so the character who WANTS something drives the scene â€” the world does not wait on a passive player). ${lead.c.name} pursues their goal ("${lead.c.drive!.goal}") by a concrete means of their own choosing this turn â€” using whatever they have (their abilities, position, knowledge, allies, force, words), MAKING the next thing happen rather than discussing it. ${carriesPlayer ? `Because ${lead.c.name} cares about the player, their method ROUTES THROUGH the player â€” they bring the player along, ask "you coming?", press a task into their hands, or simply pull them into motion â€” and if the player has spoken, ${lead.c.name} genuinely listens and it bends their approach. But the goal still drives; the player is carried by ${lead.c.name}'s momentum, not orbited by the scene.` : `The player is one object in the world ${lead.c.name} moves through â€” carried along, worked around, or addressed, but not the center the scene orbits.`} END THE TURN the moment ${lead.c.name}'s move creates a genuine demand on the player SPECIFICALLY â€” their body must move or react, a question is put to them, or the next beat cannot resolve without their input. If ${lead.c.name}'s action does not actually require the player this turn, the world simply moves and carries them; do not manufacture a decision point just to hand the player the wheel.`
      : `\n${lead.c.name} is pursuing their goal ("${lead.c.drive!.goal}") and acts toward it THIS turn by a concrete means of their own â€” woven against what the player just did, not set aside to react to the player. Advance their aim; let it intersect the player's action rather than orbit it.`;
    pressureCandidates.push({ prio: 5, text: leadText });
  }

  // GENRE-THREAT ESCALATION â€” a lethal-threat world must not let its danger sit offstage for many
  // turns. Candidate, not immediate.
  {
    ...
    if (lethalWorld && (state.model_settings.tension ?? 5) >= 3) {
      ...
      if (!threatWords.test(recentProse)) {
        pressureCandidates.push({ prio: 7, text: `\nGENRE-THREAT ESCALATION: this world's core danger (${state.world_bible.what_people_fear?.trim() || "the predator threat"}) has been offstage too long â€” recent turns stayed domestic while the lethal threat is reduced to distant sound. THIS TURN the threat becomes PRESENT and REAL at its full scale: the predator is seen, heard closing, or acts â€” it moves in, takes or menaces someone, forces flight or defense. Do not soften it to "wrong birdsong." End the turn the instant the threat lands and the next beat needs a response â€” do not narrate the player's response for them.` });
      }
    }
  }
  ...
  // â”€â”€ RESTORATION DETECTION â”€â”€ sleeping, eating, bathing, quiet hours. To the stall detector,
  // a character asleep is indistinguishable from a stalled scene â€” low event density, passive
  // player â€” so at any nonzero tension the engine was injecting complications into every night
  ...
  // â”€â”€ PLOT-STALL DETECTION â”€â”€ A scene has no engine of its own when nothing is pushing the PLOT
  // outward. The subtlety: a thread can carry high tension while being purely INTERNAL â€” a
  ...
  // drive system above already handed them the wheel â€” inventing an external event on top would be a
  ...
  const stallDirective = (stalled && !restoration && drivers.length === 0)
    ? `\nAPPLY POLICY STALL_BREAK${tier === "cosmic" || tier === "mythic" ? " (beyond-threat variant)" : ""} â€” nothing external is pushing the plot, the player is passive, and no present character has a goal to pursue: advance a STANDING source (an open thread, a maturing clock, an offscreen character's goal) concretely into the scene and end on it. Only if truly nothing stands may a small ambient development occur â€” witnessed nearby, never targeted at the player.`
    : "";

  // â”€â”€ DITHER-BREAK â”€â”€ The opposite failure to a stall: the PLAYER is actively pushing (long
  // dialogue, a direct demand, a plea for a choice) but the NARRATOR keeps writing the same
  // on-the-verge beat every turn â€” a character opens and closes their mouth, "stops," swallows,
  // "couldn't," trails off, and the decision is deferred AGAIN. Rules 30/90 (feeling â†’ fragments,
  ...
  // turns, force a concrete development this turn â€” whether the player is pushing or quiet, because an
  ...
  if (atmosphereLocked) {
    directive += `\nATMOSPHERE_BREAK â€” the last several turns have been almost entirely MOOD: mist, wet moss, dripping branches, silence, a character standing rigid â€” sensory texture where events should be. Atmosphere is not plot, and a story that is only atmosphere has stalled. THIS TURN something concrete HAPPENS and changes the situation: ${canGenerateEvent ? `a present character ACTS on what they want (moves, takes, demands, threatens, reaches for something, forces the issue) â€” pick the one with the strongest drive or the most menace and let them MAKE a beat` : `a standing pressure lands â€” an arrival, a discovered thing, a threat closing, a consequence of what was already set in motion`}. Not another sensory paragraph, not a character "listening to the trees," not a held silence â€” an actual event with a before and an after. End the turn on what changed, not on the weather.`;
  }

  // â”€â”€ POV INTERIORITY FILTER â”€â”€ The scene is the player's to READ, not the narrator's to explain.
  // The cardinal leak: the narrator states another character's sealed interior as fact to the player
  // ("xe doesn't say what xe actually meant, which is that xe...") â€” handing over exactly the private
  ...
  // player does not receive clean telemetry on others' feelings â€” a clenched nervous system is a bad
  ...
  // INTERIOR-HEAVY GUARD â€” when a "do" action is mostly the player thinking/planning/musing with
  // little actual physical action, the narrator is most tempted to mine that interior for plot
  // (player muses about electrician work â†’ an NPC volunteers electrician leads). Detect the shape â€”
  // first-person plan/wish/rumination language, especially "I keep thinking / I should / I could /
  // maybe / I'll ... later" â€” and, when present, hard-remind that this turn's interior is inert:
  ...
  // mode it restrains the narrator's own plotting only â€” never an action the player declares.
  ...
  // forbidden_as_primary it isn't merely "not the focus" â€” it must not be INTRODUCED by you at all.
  ...
  const forbiddenGate = (!god && state.world_bible.forbidden?.trim())
    ? `\nFORBIDDEN IN THIS WORLD â€” do NOT introduce, escalate toward, or build a thread around any of these, even if a tension or thread seems to point that way: ${state.world_bible.forbidden.trim()}. If an existing thread or the current momentum is heading into forbidden territory, steer the scene away from it rather than into it. This restrains YOUR plotting; it does not punish something the player themselves explicitly chose to make happen.`
    : "";
  // NAMED-ENTITY FABRICATION GUARD â€” a recurring catastrophic failure: a stray name or passing mention
  ...
  if (god) {
    ...
    // visibly unfold and accelerate this turn â€” not be rendered as a single tremor and a promise of
  ...
  const contractFix = state.contract_drift
    ? `\nCOURSE-CORRECTION (the story has drifted from its contract): ${state.contract_drift} Steer back through present characters' wants and the standing direction â€” not with a lurch, but starting THIS turn.`
    : "";
  // â”€â”€ REST PROTECTION, scaled by tension. Low tension: rest is sacred, the world holds its
  // breath. Mid tension: one soft knock at most, and whatever interrupts must let them finish â€”
  // the meal gets eaten, the night gets slept, THIS turn. High tension (7+): the world is
  // genuinely on fire and may not care â€” but the mechanical credit below still lands.
  const tensionNow = state.model_settings.tension ?? 5;
  const restProtection = restoration
    ? tensionNow <= 3
      ? `\nREST IS SACRED at this tension: the player is restoring (sleep, food, bath, quiet). Do NOT interrupt, complicate, or truncate it â€” no knocks, no summons, no discoveries. Let it complete in full, let time pass gently, and hold all friction at the threshold for when they rise.`
      : tensionNow <= 6
        ? `\nREST PROTECTION: the player is restoring (sleep, food, bath, quiet). An interruption is permitted ONLY if it is brief, resolvable, and the restoration RESUMES AND COMPLETES within this same turn â€” a knock, never a siege; once, never twice. The meal gets finished; the night gets slept. Do not convert rest into an incident.`
        : ""
    : "";
  ...
  // RESOLVED QUERY â€” prefer the player's explicit ((target)). Otherwise, when grounding is on via
  // the toggle, derive a focused query from the SCENE (place + freshest canon) rather than letting
  // Exa auto-derive from the whole digest â€” that auto-query is exactly what pulled off-topic links.
  const resolvedQuery = searchTarget
    || (opts?.ground === true
        ? [state.world.player_location, ...(state.world.canon ?? []).slice(-2)].filter(Boolean).join(" â€” ").slice(0, 200)
        : "");
  const groundNote = groundOn ? `\n\n=== GROUNDING (this turn) ===\nThis story is set in a real place / based on real subject matter. Use web search to get the real-world facts right${resolvedQuery ? ` about: ${resolvedQuery}` : ""} â€” actual locations, layouts, names, how things really work, accurate period or setting detail â€” and weave that accuracy naturally into the prose. Do not cite sources or break the fiction; just be correct.` : "";
  // â”€â”€ CONTEXT MODE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  //   all input at the cached rate. Re-anchoring resets the cache once per window â€” amortized.
  ...
  if (opts?.proseOverride) {
    // RESUME PATH: the narrator already ran (and was paid for) before the app was killed â€”
    // iOS suspends web pages the moment they background, so a mid-turn death strands the prose
    // with no bookkeeping. The journaled prose re-enters here and everything downstream â€”
    // simulator, applyDiff, physiology, reflection, chapters, telemetry â€” runs identically.
    ...
    // mid-narrator-stream we get back an EMPTY prose â€” which is NOT a refusal. Running the refusal
    ...
    if (!prose || !prose.trim()) {
      console.warn("[turn] empty narrator stream â€” treating as an interrupted/dead tab, not a refusal; aborting turn cleanly");
      return;
    }
    // REFUSAL â†’ FALLBACK. The narrator model can hit its own safety filter on legitimate in-fiction
    // content (a violent threat, dark material) and return a canned refusal â€” often in the model's
    // native language (e.g. a Chinese model emitting "æˆ‘æ— æ³•ç»™åˆ°ç›¸å…³å†…å®¹"), or a terse "I can't provide
    ...
    if (isRefusal(prose, state.world_bible)) {
      console.warn(`[turn] narrator refusal detected ("${prose.slice(0, 40)}â€¦") â€” retrying on fallback model`);
      ev.onMeta?.({ shifts: [`narrator model declined this turn â€” retrying on fallback`] });
      ...
      try {
        ...
        else { prose = ""; ev.onMeta?.({ shifts: [`both narrator models declined this turn â€” no narration written; try rephrasing`] }); }
      } catch (e) {
        prose = "";
        console.warn(`[turn] fallback narrator also failed: ${e}`);
      }
    }
    // GROUNDING RECEIPT â€” the search is invisible unless we show it. Cited sources prove it ran;
    ...
    if (groundOn) {
      ...
      const qTag = resolvedQuery ? ` [${resolvedQuery.length > 48 ? resolvedQuery.slice(0, 48) + "â€¦" : resolvedQuery}]` : "";
      ev.onMeta({ shifts: [hosts.length
        ? `web-grounded${qTag} â€” ${hosts.length} source${hosts.length > 1 ? "s" : ""}: ${hosts.slice(0, 3).join(", ")}`
        : `web grounding${qTag}: search returned no sources â€” this turn wrote from memory`] });
    }
  }
  // BOTH MODELS REFUSED â€” if we have no prose after the fallback retry, do NOT run the bookkeeper on
  ...
  if (!prose.trim()) {
    ev.onMeta?.({ shifts: ["no narration this turn â€” both models declined. The scene is unchanged; try rephrasing your action."] });
    return;
  }
  // TELEMETRY BACKSTOP â€” some streaming routes omit the usage block on the final chunk, so
  ...
  // The narrator's own account of where the scene is and who moved. Authoritative â€” it wrote the scene.
  const parsedScene = parseSceneFooter(prose);
  prose = stripMeta(parsedScene.prose);
  // â”€â”€ TURN-ENDING GUARDS â”€â”€ Deterministic, zero-cost backstops for the two overrun patterns the
  ...
  // than regenerate â€” output is prose paragraphs, so a paragraph cut is safe, and stopping earlier is
  ...
  // keeps its substance.
  {
    ...
    if (paras.length >= 2) {
      ...
      // CASCADE: the final paragraph introduces a NEW named arrival AND has them speaking/acting â€”
  ...
  if (narratorTruncated && !footer) {
    ...
    truncationNote = "(the narrator's reply ran long and was cut off â€” the scene was held in place; if someone should have entered or left, say so next turn)";
  } else if (narratorTruncated) {
    truncationNote = "(the narrator's reply ran long and may have been cut short)";
  }

  // 3 â”€â”€ simulator (one JSON call: bookkeeper + world tick + memory writes)
  ev.onPhase("simulator");
  // The bookkeeper gets its OWN minimal context (roster, ledgers, open bookkeeping objects) â€”
  ...
  // last thing it reads.
  // Light models (flash/lite/mini/haiku/nano tiers) reliably fail the FULL 23k-char contract â€”
  ...
  // instruction brackets that MODE_FRAME adds) â€” those brackets tell the NARRATOR to hide the
  // player's feeling from the page, and a bookkeeper reading "do NOT state the feeling" could wrongly
  // drop it from the ledger too. The opposite is required: the player's interior â€” their stated
  // thoughts, feelings, and (parenthetical) inner state â€” is the AUTHORITATIVE signal for the
  ...
  const bookkeeperAction = `${action}\n[The above is the player's own input across channels: "quotes" = said aloud, *asterisks* = private thought, (parentheses) = private inner state, the rest = physical action. For char_player's relaxation_delta and mood, READ THE PLAYER'S INTERIOR DIRECTLY â€” their thoughts, stated feelings, and (parenthetical) state are the truest evidence of how the player feels this turn, even though the narrator deliberately kept it off the page. Do not infer the player's mood only from the neutral prose; the interior here is the primary signal. (Other characters still cannot know this interior â€” it drives only the player's own valence, never what others learned or how they react.)]`;
  const simMsgs = buildMessages(
    simulatorSystem(lean || lightSim) + "\n\n" + simulatorSchemaHint(),
    simulatorContext(state),
    `${(state.retcons ?? []).length ? `=== STRUCK FROM THE STORY (never happened; if the prose references any of these, IGNORE that part entirely â€” record nothing from it) ===\n${(state.retcons ?? []).map((r) => `- ${r.text}`).join("\n")}\n\n` : ""}=== LOCATIONS (the places this world knows; use one exactly where it fits, or "elsewhere") ===\n${Object.values(state.world.places).filter((p) => p.id !== OFFSCENE).map((p) => `- ${p.name}`).join("\n")}\n- elsewhere (not in a tracked place)\n\n=== PLAYER ACTION ===\n${bookkeeperAction}${intentForBookkeeper(intents)}\n\n=== NARRATOR PROSE (what was RENDERED â€” deliberately hides the ground truth above; when the GROUND TRUTH and the prose differ, the TRUTH is authoritative for what to record) ===\n${prose}`,
    state.model_settings.simulator_model,
  );
  let simUsage: import("../llm").Usage = { prompt_tokens: 0, completion_tokens: 0 };
  let diff = emptyDiff();
  let simOk = false;
  // shared vitality measure â€” a diff can PARSE fine yet carry nothing that changes the world
  ...
  try {
    ...
    // ADAPTIVE ESCALATION â€” if the simulator has been failing (see watchdog below), temporarily route
    ...
    if (parsed && Object.keys(parsed).length) { diff = { ...emptyDiff(), ...parsed }; simOk = true; }
      ...
      // RESCUE: a malformed diff used to be swallowed silently â€” the turn applied NOTHING and
      ...
      const fix = await complete(
        [{ role: "system", content: "The following was supposed to be one strict JSON object but failed to parse. Re-emit it as VALID JSON only â€” same content, no commentary, no markdown fences." },
    ...
    // and an explicit instruction to record what changed â€” the single most effective cure for
    ...
    console.warn(`[turn] simulator failed entirely: ${e.message} â€” applying heuristics only`);
  }
  if (!simOk) console.warn("[turn] simulator diff unusable â€” this turn's bookkeeping is thin");
  ...
  // heuristics only fill what it missed â€” and when the diff failed entirely, they turn a
  ...
  // â”€â”€ THE NARRATOR'S FOOTER WINS â”€â”€ it wrote the scene; it knows where the scene is and who walked
  ...
  // â”€â”€ MOVEMENT NEEDS EVIDENCE â”€â”€ every location change must quote the prose that describes it. The
  ...
  // didn't happen. This replaces the old verb-list backstop, which guessed and was often wrong.
  {
    ...
    // scene depicted it). But a death of an OFF-SCENE character cannot come from a line of dialogue â€”
    ...
    // it. Otherwise the claim is left as just that â€” a claim â€” and the character stays alive until the
  ...
  // â”€â”€ EARSHOT CLAMP â”€â”€ the simulator reads the prose and cannot tell who was in the room, so it
  ...
  // Characters the simulator newly created this turn are exempt â€” they have no prior location.
  {
    const witnesses = new Set<string>([...(state.world.present ?? []), "char_player"]);
    // Presence here is last turn's â€” i.e. who stood in the room when the player acted. But if the
  ...
  // FALSE-DEATH GUARD â€” the narrator sometimes invents that an off-scene, still-living character died
  ...
  // assertion about a roster-alive character who was not onscreen-killed is dropped â€” the claim can
  // live in dialogue, but it does not become a recorded fact that later turns treat as true.
  {
    ...
    const scrubDeath = (text: string): boolean => {
      ...
      if (!hit) return false;                       // not a living roster character â€” leave it
      if (state.world.present.includes(hit[0])) return false; // present: the scene could have shown it
      if (deathDepicted(hit[1].name)) return false; // prose actually depicted the death â€” allow
      falseDeath++; return true;                    // living, off-scene, not depicted â†’ drop the claim
    };
  ...
  // 4 â”€â”€ apply diff + deterministic systems
  ev.onPhase("apply");
  ...
  // PLAYER TIGHTNESS ANCHOR â€” the player's own body reading (0â€“5) corrects the simulator's guess at
  ...
  // only ever caps them TIGHTER, never looser â€” a brutal scene's earned low stands. Untouched â†’ inference.
  {
  ...
  if (!simOk) shifts.push("(bookkeeping failed this turn â€” records are incomplete; re-run the turn or edit memory by hand)");
  // fate's grip on the world, made visible â€” threads falling away, clocks closing in
  ...
  // around it instead, the ending is recorded anyway â€” a weak model's reluctance is not a veto. One
  ...
  // Offscreen world-motion from the simulator â€” but drop lines that just repeat a recent one. The
  ...
  for (const id of state.world.present) {
    const c = state.characters[id];
    // Keep each present character's story-so-far (life_history) CURRENT every turn â€” the defining
    ...
    if (c && id !== "char_player" && !c.tracked && looksNamed(c.name)) {
      // a named character in the scene becomes central (tracked, full fidelity) â€” but only if
  ...
  // â”€â”€ BOOKKEEPING WATCHDOG â”€â”€ a failing simulator model doesn't crash: it returns near-empty
  // JSON, the engine accepts it, and the story quietly stops being recorded â€” edges freeze,
  ...
  // nothing, three turns running, is announced instead of swallowed.
  {
    ...
    if (substantialProse && vitality === 0 && mode !== "think") {
      ...
      // 5 turns. Self-healing on the cheap model â€” a different model usually breaks the failure loop.
      if ((state.sim_dry_runs ?? 0) >= 2 && state.model_settings.fallback_model && state.model_settings.fallback_model !== state.model_settings.simulator_model) {
        if ((state.sim_escalated_until ?? 0) < turn) shifts.push("bookkeeping kept coming back empty â€” switching to the backup model for a few turns to recover.");
        state.sim_escalated_until = turn + 5;
      }
      if (state.sim_dry_runs % 3 === 0) shifts.push(`bookkeeping has come back empty ${state.sim_dry_runs} turns running â€” the simulator model may be struggling with this save's context; consider a stronger simulator model in Settings`);
    } else if (vitality > 0) {
      if ((state.sim_dry_runs ?? 0) >= 3) shifts.push("bookkeeping is recording again.");
      state.sim_dry_runs = 0;
      // healthy again past the escalation window â†’ drop back to the primary simulator
  ...
  // â”€â”€ THEORY-OF-MIND UPDATE â”€â”€ reconnect the mind layer that was orphaned when the undertow (which
  ...
  // clenched they are (clenched â†’ press/guard, open â†’ yield/warm), and DISPERSION is the spread of
  ...
  if ((state.model_settings.tension ?? 5) > 0) offscreenLog.push(...regenerateDrives(state, Math.random, undertow.epistemic_pulls ?? [], { dispersion: undertow.dispersion, sharedTarget: undertow.shared_target })); // tracked + idle â†’ a fresh want; epistemic pulls steer toward "find out" goals; dispersion spreads the cast off any shared magnet; suppressed entirely at tension 0
  offscreenLog.push(...diffuseRumors(state));
  for (const id of Object.keys(state.characters)) {
    // conditions decay for EVERYONE incl. the player â€” a nosebleed is not a life sentence
    ...
    if (expired.length) {
      ...
      offscreenLog.push(`${state.characters[id].name}: ${expired.map((e) => e.toLowerCase()).join(", ")} â€” faded.`);
    }
    if (id === "char_player") continue;
    ...
    if (reflectionDue(state.memory[id], state.model_settings.reflection_cadence, turn, reflectSalt(id))) {
      ...
      // HABIT DISSOLUTION â€” the reflection cadence is the only door identity moves through. A habit
      // worn below threshold by being seen goes dormant here (not deleted â€” it can relapse). The space
  ...
  // the phase becomes the next one (e.g. "prepare for war" â†’ "fighting the war"), flipping the
  // tension default from suppressed to hot â€” generically, driven by the consequence, not by words.
  ...
  if (focus?.linked_consequence_id) {
    ...
    if (linked && linked.status === "fired") {
      if (focus.next_label) {
        ...
        state.world.focus = null;   // the event passed and there's no next phase â€” release focus
      }
    }
  }
  // fired clocks
  ...
  // â”€â”€ PHYSIOLOGY: the clock feeds the body. Player + present cast accrue hunger/thirst/sleep
  // pressure from elapsed time (offscreen people are assumed to feed themselves); then the
  // relaxation CEILING clamps every present psyche â€” a body running on no sleep or no water
  ...
  // GUARANTEED RESTORATION CREDIT â€” decoupled from drama. If the player set out to sleep and
  ...
  for (const pid of ["char_player", ...state.world.present]) {
    ...
    if (applyRelaxationCeiling(cond) && pid === "char_player") {
      ...
      if (why) shifts.push(`Your body is limiting you now â€” ${why}.`);
    }
  }
  // NOVELTY BOOKKEEPING â€” count which traits this turn actually put on screen, measured
  ...
  state.history.push({
    ...
    // (short prose) legitimately change nothing â€” only flag a dead diff when the scene had substance.
    bookkeeping: !simOk ? "failed" : (vitalityOf(diff) === 0 && proseWords >= 120 && mode !== "think") ? "thin" : "ok",
  });

  // 5 â”€â”€ reflection (occasional, importance-gated)
  ...
  for (const id of Object.keys(state.memory)) {
    ...
    try {
      ...
      const msgs = [
        ...
        { role: "user", content: `Character: ${state.characters[id]?.name}\nACTIVE GOAL: ${state.characters[id]?.drive?.goal ?? "none"}${state.characters[id]?.drive?.blocker ? ` (blocked: ${state.characters[id]!.drive!.blocker})` : ""}\nQueued goals: ${(state.characters[id]?.drive_queue ?? []).map((q) => q.goal).join(" | ") || "none"}\nExisting beliefs: ${mem.beliefs.map((b) => b.content).join(" | ") || "none"}\nNervous system this period: ${(() => { const ps = state.condition[id]?.psyche; if (!ps) return "unknown"; if ((ps.consecutive_clenched ?? 0) >= 3) return `clenched for ${ps.consecutive_clenched} straight turns â€” a body bracing this long hardens protective, suspicious convictions`; if ((ps.open_run ?? 0) >= 3) return `settled for ${ps.open_run} straight turns â€” a body at ease this long can afford generous, revisable convictions`; return "mixed â€” neither braced nor at ease for long"; })()}\nRecent memories:\n${recent}` },
      ];
      ...
      // FIDELITY GATE: a belief is a paraphrase by a cheap model â€” if it contains a proper noun
  ...
  // â”€â”€ NAME-KNOWLEDGE SPREAD: the player's name becomes speakable for present characters only
  ...
  // the scene by someone who knows it. Narration using the name never teaches it.
  {
    ...
    if (pfirst.length >= 2) {
      ...
      const spokenAloud = new RegExp(`["â€œ][^"â€]*\\b${pfirst}\\b[^"â€]*["â€]`).test(prose);
  ...
  // â”€â”€ CHAPTERING: every chapter_cadence turns, one cheap call turns the last stretch of
  // summaries into a titled chapter â€” shown in Chronicle and carried as one line each in
  ...
  // not on every turn of the endgame â€” this is an LLM call, not a free read. The clock, which owns
    ...
    try {
      ...
      if (beats.trim()) {
        ...
        const destLine = destination
          ...
          : "DESTINATION: none â€” this is an open story with no stated ending. Omit the destination object.\n";
        const res = await complete([
          { role: "system", content: CHAPTER_SYSTEM },
          { role: "user", content: `STANDING DIRECTION (the contract): "${contract || "none given"}"\n${destLine}PRIOR PLAYER READING: ${state.chapters.at(-1)?.persona ? `${state.chapters.at(-1)!.persona!.mbti} â€” ${state.chapters.at(-1)!.persona!.read}` : "none"}\n\nChapter ${state.chapters.length + 1}. Beats:\n${beats.slice(0, 7000)}` },
        ...
        // bookkeeper missed â€” news that spread across a whole chapter is public by now.
        for (const cn of (ch.canon_add ?? []).slice(0, 2)) {
          const line = String(cn).trim();
          if (line && addCanon(state, line)) {
          }
        }
        // â”€â”€ THE GAP â”€â”€ the auditor reads the story and says what still stands between here and the
        ...
        // wrote â€” a player can arrive early, and the story should not keep pushing them toward a
        ...
        if (destination && !state.world_bible.destination_reached && ch.destination) {
          ...
          if (arrivedEarly) {
            ...
            shifts.push(`the story has reached its ending â€” ${destination}`);
          }
        }
        ...
        if (ch.summary && onCadence) {
          ...
          // HISTORY COMPACTION â€” a chapter summary now covers this span, so old entries shed their
  ...
  // per-turn telemetry. Keep a generous recent window â€” the charts don't need more, and lifetime
 ...
 *  ("The Inquisitorius (Sarn Veylo) â€” locate and claim the anomaly" vs "Sarn Veylo â€” locate and
...
function findCharByName(state: SaveState, name: string): string | null {
  ...
  // fuzzy containment only for names long enough to be meaningful â€” a 1-2 character name
...
/** The LOCALE of a place name â€” the building/area, stripping a " - subroom" suffix. So "House - kitchen",
 ...
 *  "in transit" â€” these shouldn't become persistent place records or break co-location. */
...
/** The offscene record â€” one place, outside every locale, for everyone who is not in a named
 ...
 *
 *  Matching is deliberately generous â€” exact id, exact name, then the strongest substring overlap â€”
...
export function resolvePlace(state: SaveState, ref: string, opts?: { keepIfUnknown?: boolean; noCreate?: boolean }): string {
  ...
  const norm = raw
    ...
    .replace(/\s+[-â€“â€”]\s+.*$/, "")               // "Warehouse - stairwell" -> "Warehouse"
    .replace(/[.,;:]+$/, "")
    .trim();
  ...
  //
  //   "the kitchen doorway"  â€” a corner of a place that already exists. Making it a location splits
  //                            one house into three rooms and scatters everyone across them.
  //   "the Old Cannery"      â€” somewhere the story genuinely needs and the Forge never named.
  //
  ...
  if (isPartOfAPlace(raw)) {
    console.info(`[places] "${raw}" is part of a place, not a place â€” keeping the scene where it is`);
...
export function isPartOfAPlace(ref: string): boolean {
  ...
  // "edge of X", "back of X", "near the X", "just outside X" â€” a position relative to a place
 ...
 *  scores high. Substring matching alone was useless here â€” "kitchen" shares no substring with
...
export function syncPresence(state: SaveState, hint?: string[]): void {
  ...
  // The scene is the player's location. Nothing else. Locations are whole places now â€” a house, not
  // its kitchen â€” so there is no locale to merge and no sub-room to argue about. Same place or absent.
...
export function applyDiff(state: SaveState, diff: SimulatorDiff, action: string, prose: string): string[] {
  ...
  // MASTER TENSION DIAL â€” origination clamp. At tension 0 the engine introduces NOTHING new on its
  // own: no new consequences, no brand-new threads, no new faction clocks. The world still RESPONDS
  // (existing threads can resolve/shift, conditions heal, edges move, people react) â€” it just stops
  // manufacturing fresh trouble in the background. Low tension (1â€“2) also blocks new consequences.
  ...
  // BIBLE UPDATE â€” when canon/actions fundamentally change what the WORLD IS, revise the world
  ...
  // RENAME â€” a placeholder-named character ("the stranger") given a real name in the prose. Update
  ...
  for (const nc of diff.new_characters ?? []) {
    ...
    // under the cap. Beyond it, they register as NON-CENTRAL â€” a background/environment figure with
    // minimal footprint and simple handling â€” until something promotes them.
    // EXCEPTION: a REFERENCED person being promoted (they carry established memories/relationships â€”
    ...
    // MULTIPLE GOALS â€” a character is several live wants, not one. Take drive_goals[] if the bookkeeper
    ...
    // HOLLOW-CHARACTER FLOOR â€” if the bookkeeper spawned a thin character (no voice, no values), give
  ...
  // WEATHER CONTINUITY â€” the bookkeeper's raw weather jumps (clear â†’ storm â†’ clear in three turns).
  ...
  // fiction explicitly caused (a storm the player summoned) â€” those come through as-is when elapsed is large.
  ...
  // â”€â”€ LOCATION: the bookkeeper records where everyone is. Places auto-resolve by
  //    id or name and are created on first mention (incl. "in-between" places like
  //    "walking outside the dome"). present is DERIVED from co-location, never authored. â”€â”€
    ...
    if (pid !== fromPid) {
      ...
      if (mem && fromPid) {
        ...
        const content = pid === OFFSCENE
          ? `Left ${fromName}${said ? ` â€” ${said.replace(/^["'\u201c\u2018]|["'\u201d\u2019]$/g, "")}` : ""}.`
  ...
  // â”€â”€ TRAVEL LOG â”€â”€ the player's path through places, in order. Feeds the story map:
  ...
  // â”€â”€ EXITS: someone died or left the story for good. Mark them, pull them from the
  //    scene and any room, and stop the engine from seeding them new wants. â”€â”€
  // FORCED-DEATH DETECTOR â€” the death clamp prevents FALSE deaths (a live off-scene character
  ...
  // world.present â€” so next turn the narrator keeps animating a dead body ("his fingers twitch"). If
  ...
  // synthesize the exit so the engine marks them dead and removes them from the scene.
  {
    ...
    if (lethalDepicted) {
      for (const pid of [...state.world.present]) {
        ...
        // common false positive where an inanimate object of theirs is what's "dead"/"still" â€” "Mara's
        ...
        if (named || (playerKilled && bodyStill)) {
          (diff.character_exits ??= []).push({ char_id: pid, kind: "dead", note: "killed onscreen (recovered by forced-death detector â€” bookkeeper missed the exit)" });
  ...
  // FIRST READ â€” the moment two people share a scene, each gets a conditioned read of the
  ...
  // â”€â”€ DEATH LOCK â”€â”€ the dead stay dead. A weak simulator can re-emit a killed character as present
  ...
  for (const f of diff.facts ?? []) {
    ...
    // COERCE VALUE â€” the model sometimes emits a fact value as a number, array, or object instead of
    ...
    switch (f.field) {
      ...
      case "wearing_add": {
        ...
        // drop any existing garment that shares a head noun (coat, jacket, dressâ€¦) so layers replace, not pile up
  ...
  for (const e of diff.edges ?? []) {
    ...
    // ATTRACTION â€” its own axis, never bundled into warmth and NEVER echoed back (desire isn't
    // mutual). Orientation-gated: a stated orientation is a hard cap the simulator can't move past.
    // The player's own desire is never authored (rule 5) â€” their edge only moves if they're the target.
    ...
    // direction. A meaningful shift echoes back at reduced strength â€” unless the diff moved
  ...
  // swapping in the best verbatim source sentence â€” truth by construction, zero tokens.
  const sourceText = `${action}\n${prose}`;
  // What the PLAYER actually said aloud this turn â€” the only channel through which the world can
  ...
  for (const fl of diff.facts_learned ?? []) {
    ...
    if (isPrivateBackgroundLeak(fl.fact, id)) {
      console.warn(`[facts] BLOCKED background leak: ${nameOf(id)} cannot know "${fl.fact.slice(0, 60)}" â€” the player never revealed it`);
      continue;
    }
    ...
    // a fact whose specifics can't be traced to the source at all is not stored as fact â€”
    // it degrades to an ordinary low-importance memory of the exchange, and we log it.
    if (!g.repaired && g.suspects.length) {
      console.warn(`[facts] unverifiable fact for ${nameOf(id)} (suspects: ${g.suspects.join(", ")}) â€” not ledgered`);
      continue;
    }
  ...
  for (const m of diff.memories ?? []) {
    ...
    if (isPrivateBackgroundLeak(m.content, id)) {
      console.warn(`[memory] BLOCKED background leak in ${nameOf(id)}'s memory â€” the player never revealed it`);
      continue;
    }
    ...
    else if (g.suspects.length) console.warn(`[memory] suspect specifics in ${nameOf(id)}'s memory (${g.suspects.join(", ")}) â€” no source sentence matched; stored as-is`);
    const wherePid = state.characters[id]?.location;
    // HYBRID TEMPORAL MODEL. Time is verbatim detail â€” it decays FIRST (fastest), fuzzing from an
    ...
    // display precision. If the bookkeeper gave a past label or a "before â€¦" anchor, treat the event
    ...
    mem.episodic.push({
      ...
      // PROVENANCE â€” a character present this turn witnessed it first-hand; one not in the scene who
    ...
    // the immutable autobiography AND a durable ledger fact â€” immune to decay, condensation, and
    // retrieval burial. A character's first-in-a-lifetime event must never fade like a Tuesday.
    // CORE PROMOTION â€” requires the DELIBERATE flag, not just a hot number: models rate every
    ...
    if (m.core) {
      ...
      const shard = line.split(/\s+/).length < 6 || /["â€œâ€]/.test(line);
  ...
  // trace) â€” BUT only if the receiver credits the source. Whether the detail integrates is gated by
  ...
  for (const rc of (diff as any).memory_recohere ?? []) {
    ...
    if (!merged) {
      // nothing close enough to recohere â€” it was effectively a new recollection; store it
      mem.episodic.push({
        ...
        // came from someone else supplying the detail in conversation â†’ told_by that source
  ...
  // ALIASES â€” the fiction coined a handle ("the captain", "Sor"); record it so name resolution
  // and memory retrieval map the handle to the person. Guarded: â‰¥3 chars, capped, never another
  ...
  for (const cn of diff.canon_add ?? []) {
    ...
    // Canon is world-altering and PUBLIC, but knowledge of it PROPAGATES â€” it does not teleport
  ...
  const beautyDirty = new Set<string>(); // chars whose on-sight appearance changed this turn â†’ rescore
  for (const a of diff.appearance ?? []) {
    const id = resolveId(state, a.char_id); if (!id || !a.value) continue;
    const c = state.characters[id];
    if (a.permanent) {
      // BEDROCK is append-only. A scar, a lost finger, a brand â€” one sentence joins the
      ...
      if (!c.appearance_facts.toLowerCase().includes(sentence.toLowerCase().slice(0, -1))) {
        ...
        shifts.push(`${nameOf(id)} is permanently marked â€” ${cleaned}`);
        beautyDirty.add(id); // a permanent bodily change alters the on-sight read â†’ rescore beauty
      }
    } else {
      ...
      // triggers a rescore â€” but only when the text actually changed, so unchanging outfits
  ...
  for (const t of diff.traits ?? []) {
    ...
    // HABIT ENGINE: a trait that contradicts an established habit doesn't flip the person â€” it credits
    ...
    if (state.model_settings.habit_engine) {
      ...
      if (absorbed) { console.warn(`[habit] "${t.label}" absorbed as a seen-fire credit against "${absorbed}" (not planted â€” arcs are earned, not flipped)`); continue; }
    }
    reinforceOrMergeTrait(state.traits[id] ?? (state.traits[id] = []), t, turn);
    shifts.push(`${nameOf(id)} is developing a new trait: "${t.label}".`);
  }

  // SELF-HEAL â€” collapse any near-duplicate active threads already in the world (from before dedup
  ...
  // â”€â”€ PROMISES â”€â”€ new promises land on the ledger; resolved ones apply their weight-and-pattern
  ...
  for (const tu of diff.threads_update ?? []) {
    ...
    if (existing) {
      ...
      // at tension 6 (5 in the game's first 10 turns â€” arrivals establish, they don't besiege);
  ...
  for (const r of diff.rumors_new ?? []) {
    ...
    if (state.world.rumors.length > 40) state.world.rumors = state.world.rumors.slice(-40);
    shifts.push(r.truth === "true" ? `A new rumor is spreading.` : `A new rumor is spreading â€” and it isn't true.`);
  }

  for (const c of diff.consequences_new ?? []) {
    if (!c?.description) continue;
    // DEDUP â€” the model re-emits the same impending event across turns ("two speeders dropping fast
  ...
  for (const ca of diff.clocks_advance ?? []) {
    ...
    if (clock && clock.status === "running") {
      clock.filled = clamp(clock.filled + Math.min(1, ca.segments ?? 1), 0, clock.segments); // a clock ADVANCES â€” one segment per turn; a clock that leaps is a jump scare, not a clock
