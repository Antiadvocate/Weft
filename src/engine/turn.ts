...
function echoBan(_state: SaveState): string {
  ...
  return `\nDIALOGUE COMES OUT OF THE SPEAKER'S OWN HEAD: no character restates, describes, recaps, or marvels at what the player just did. THE TEST ON A FINISHED LINE: take the player's last action out of the scene and read the line again. If it no longer means anything, it was built out of that action rather than out of the speaker, and it is rewritten. This covers every wording of it. Astonishment is real and shows in what a person DOES: they stop walking, they lose their place, they follow, they leave, they ask something adjacent, they carry on with what they were doing and get it slightly wrong. Every spoken line originates in the speaker's own want, their own errand, their own body, or something they were already thinking about before this happened — a character with nothing of their own to say says nothing and does something instead.`;
}
...
export async function runTurn(state: SaveState, action: string, ev: TurnEvents, mode: ActionMode = "do", opts?: { ground?: boolean; eco?: boolean; proseOverride?: string; tightness?: number; signal?: AbortSignal;
  ...
  // HOW WIDE THE ATTENTION IS. Relaxation already decides how accurately a person sees, how an
  // emotion resolves, and whether a habit can be caught; it did not decide how much of the world
...
JUXTAPOSITION: observable detail and any conclusion sit side by side without a connective. Never join them with a verb of perception or cause. Write the observable thing, end the sentence, and let the next sentence be the next thing that happens rather than a gloss on the last one.`;
  ...
  const contractFix = state.contract_drift
    ? `\n\n[THIS STORY HAS DRIFTED FROM WHAT IT WAS MADE TO BE.\nWhat it drifted into: ${state.contract_drift}\n`
      ...
      + `THE REST OF THE CORRECTION IS AN OPENING, AND NOTHING HERE IS OWED. Let this world be the kind of story it is again — through what the people here already want, at whatever strength the scene honestly supports. A turn with none of it in it is allowed; several are, if that is where the player has taken this.\n`
  ...
  // THE SHARED RECORD. Characters hold private memories and nothing was ever held in common, so a
  // false account could not be contradicted by the four people who watched it not happen. See
  // engine/witness.ts.
...
export function resolvePlace(state: SaveState, ref: string, opts?: { keepIfUnknown?: boolean; noCreate?: boolean }): string {
  ...
  if (isPartOfAPlace(raw)) {
    console.info(`[places] "${raw}" names part of a place — keeping the scene where it is`);
...
function spawnNamed(state: SaveState, action: string, shifts: string[]): string {
  ...
  return `\nNEWLY NAMED — the player just referred to ${names.join(" and ")}, who has no history in this world yet. Bring them into the scene as a WHOLE PERSON on the page: a specific body, a way of speaking that is theirs, wants that predate this moment and have nothing to do with the player. They were living a life before this turn and will be after it, and they did not come here to answer anybody. Do not explain who they are to the player, and do not have them announce themselves — write them as though they have always been in this story.`;
}
...
function unregisteredSpeakers(state: SaveState, prose: string, action = ""): string[] {
  ...
  for (const nc of diff.new_characters ?? []) {
    ...
    const attachment = ["secure", "anxious", "avoidant", "disorganized"].includes(aStyle)
      ...
      : undefined;
    /* WHAT THE BOOKKEEPER IS ALLOWED TO PUT IN A NEW PERSON'S MOUTH.
     *
  ...
  // SOMETHING THE ROOM COULD NOT EXPLAIN. Applied before presence is rebuilt so the witnesses are
  // still the people who were standing there when it happened. See engine/reaction.ts.
  for (const line of applyUnexplained(state, (diff as { unexplained?: { what: string; witnesses?: string[] } }).unexplained, turn)) shifts.push(line);
  ...
  for (const t of diff.traits ?? []) {
    ...
    /* A TRAIT LABEL IS PRINTED ON THE CARD EVERY TURN THIS PERSON IS IN A SCENE, under "Acquired",
     * next to the core traits and the voice. So a label written as a general statement about people
     * — "kindness that keeps coming back", "needling as a way of making contact" — is not one bad
     * string in the ledger: it is a sample of the register, sitting in the prefix, in the document
     * the narrator writes this character out of.
     *
     * Refused rather than rewritten, on the argument this block already makes three paragraphs
     * down: if the disposition is real the prose will show it again, and the next label for it can
     * name an act. The behavioural note is only reported — it is one sentence of explanation and
     * the label is what gets quoted. */
    { const f = figureIn(t.label); if (f) { console.warn(`[traits] refused "${t.label}" for ${nameOf(id)} — ${f.why}`); continue; } }
    if (figured(t.behavioral_impact)) console.info(`[traits] "${t.label}" carries a figured behaviour note: ${String(t.behavioral_impact).slice(0, 90)}`);
