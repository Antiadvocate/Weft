...
export function neglectCue(d: NPCDrive, turn: number): string {
  ...
  const n = neglectOf(d, turn);
  return dread >= 0.7
    ? `${n} turns of not dealing with it, and it has stopped being a plan — they flinch off the subject, get short when it is near, and do the easy thing in front of them instead`
    : dread >= 0.45
      ? `${n} turns of putting it off; it is heavier now than the thing itself warrants, and they would rather be given something else to do`
...
export function tickNeglect(state: SaveState, turn: number): string[] {
  ...
  for (const id of state.world.present ?? []) {
    ...
    log.push(`${c.name} is reminded of what they have been putting off, and it lands.`);
  }
  return log;
}
...
export function liveWant(state: SaveState, id: string, turn: number): { goal: string; why: string } | null {
  ...
  const why = spent && dodging
    ? `they have not the reserve for the heavier one, and this is what they can face`
