...
/**
 * THEORY OF MIND â€” the active-inference belief layer.
 *
 ...
 * good and believability quietly dies â€” real social life runs on information
 ...
 * (b) reduce its own uncertainty. Surprise â€” the error between what it predicted and
 * what actually happened â€” is the master signal. Here:
 *
 *   â€¢ predicted_warmth / predicted_stance = the agent's generative model of the other.
 *   â€¢ each turn we compare the model to what the diff ACTUALLY did (the edge moves,
 *     the stance the QRE layer sampled) and compute prediction error.
 *   â€¢ error magnitude â†’ `surprise`, which (1) feeds the cusp load term `a` in the
 *     undertow (surprise IS the body bracing against an unpredictable world â€” the
 ...
 *     the drive system toward an EPISTEMIC goal â€” go find out â€” which the existing
 *     QRE stance layer then executes.
 *   â€¢ the model then UPDATES toward what it observed (precision-weighted: confident
 *     models move less, surprised models move more â€” Bayesian-flavored, deterministic).
 *
 * All zero-token. A few scalars per tracked character, updated like the rest of the
 * undertow. The `held_false` belief â€” one concrete thing the agent is WRONG about â€”
 ...
 *  Kept sparse on purpose â€” a theory of mind about everyone is neither realistic nor cheap. */
...
function blankBelief(target: string, turn: number, trueWarmth: number): BeliefAbout {
  return {
    target,
    // first read is a noisy, attenuated version of the truth â€” people start with a rough guess
...
// â”€â”€ INTERPRETATION-UNDER-AFFECT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// No mind perceives a raw event. The reader sees whatever the actor did THROUGH the
// reader's own edge toward that actor â€” warmth, attraction, and attachment style bend
...
// with no author input. Runs playerâ†’NPC and NPCâ†’NPC, both signs.
//
// The distortion is a signed, magnitude-scaled bend of the objective valence:
//   â€¢ low warmth toward the actor COMPRESSES neutral/ambiguous acts toward a hostile read
//     (a warm gesture from someone you distrust looks like a move).
//   â€¢ high attraction toward the actor PULLS neutral toward the intimate/invested read
//     (you find the meaning you want to find).
//   â€¢ attachment style shapes HOW the bend behaves, it is not flavor:
//       - secure: light bend, quick to take the act near-straight.
//       - anxious: amplifies the negative direction â€” reads abandonment into ambiguity.
//       - avoidant: compresses warmth specifically â€” intimacy gets flattened to neutral/mild threat.
//       - disorganized: WIDENS VARIANCE â€” the same input swings threatâ†”desire across turns,
...
function perceivedValence(objective: number, edge: PerceptEdge, turn: number): number {
  const w = edge.warmth / 100;      // -1..1 readerâ†’actor warmth
  const a = edge.attraction / 100;  // -1..1 readerâ†’actor desire
  // ambiguity weight: the bend acts hardest on near-neutral events, barely on extremes â€”
...
// â”€â”€ BREAKTHROUGH THRESHOLD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// percept filter is BYPASSED and the mind takes the objective valence straight â€” reality
// broke through. The cutoff is per-character: most minds are recoverable (a moderate cutoff),
// but a few are sealed â€” pushed so high that no ordinary contradiction lands. Sealing is
...
function breakthroughCutoff(edge: PerceptEdge): number {
  ...
  // hostility raises the wall â€” the more negative the edge, the harder truth must hit.
  ...
  // sealed: extreme negative edge + guarded style pushes the cutoff past 1.0 (unreachable by errorâˆˆ[0,1])
...
export function updateMind(
  ...
  turn: number,
  dispersion = 0, // 0..1 from the undertow: how hard the cast is pulling apart â€” erodes settled confidence
  ...
  for (const target of targets) {
    ...
    // â”€â”€ PERCEPT (interpretation-under-affect) â”€â”€
    ...
    // â”€â”€ PREDICTION ERROR â”€â”€ computed on the PERCEPT, not the truth.
    ...
    // surprise is a leaky accumulator â€” decays in calm, spikes on violation
    ...
    // â”€â”€ EPISTEMIC PULL â”€â”€ low confidence about someone who matters â†’ a want to FIND OUT
    ...
    // â”€â”€ BELIEF UPDATE â”€â”€ move the model toward what was PERCEIVED (seenWarmth), not the raw truth â€”
    ...
    // confidence rises when predictions land, falls on shock â€” but the GAIN is capped low when the
    ...
    // perfectly. This is what breaks the omniscient-chorus fixed point â€” confidence can no longer
    ...
    // concrete false belief â€” the misunderstanding that can drive a scene. The gap is measured against
    ...
    if (b.surprise > 0.6 && Math.abs(seenWarmth - b.predicted_warmth) > 35 && !b.held_false) {
      ...
      const tname = target === "char_player" ? "the player" : state.characters[target]?.name ?? "them";
      const sname = state.characters[id]?.name ?? id;
      lines.push(`${sname} finally sees ${tname} clearly â€” the misunderstanding clears.`);
...
export function mindDigest(state: SaveState, id: string): string {
  ...
  for (const b of model.about) {
    ...
    if (b.held_false) out.push(`acts as if the player ${b.held_false} â€” let this false read drive their behavior and word choice; NEVER state the belief in prose, only show them acting on it`);
    else if (divergence > 25) out.push(`treats the player as ${b.predicted_stance === "unknown" ? "an unknown quantity" : b.predicted_stance === "ally" ? "warmer than they truly are" : "more hostile than they truly are"} â€” behavior follows their read; do not narrate the misjudgment`);
    if (b.surprise > 0.45) out.push(`is freshly thrown â€” the player just did something against their expectation; SHOW the recalibration, don't state it`);
    if (b.confidence < 0.25 && Math.abs(trueWarmth) > 25) out.push(`can't get a clean read on the player â€” SHOW it as watchfulness or probing, never as narrated confusion`);
  }
...
export function epistemicGoal(state: SaveState, target: string): string {
  const tname = target === "char_player" ? "the player" : state.characters[target]?.name ?? "them";
  return `find out where ${tname} really stands â€” test them, probe, get a read`;
}
