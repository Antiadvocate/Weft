...
/**
 * Desire â€” attraction as its own axis, separate from warmth. Deterministic, zero tokens.
 *
 ...
 *      shaped by prior conditioning â€” what a person's world and history trained them to
 ...
 *      (relaxed) person, expresses cleanly â€” real flirtation, play, initiative â€” or is
 ...
 *      overcorrection), and held long enough it hardens into fixation â€” a craving state
 ...
 *      own â€” nothing gripping it, it liberates itself. Same energy, two trajectories,
 ...
 *      over time ("made up for it") â€” but if the conditioned first read was flat, the drift
...
/**
 * Intrinsic attractiveness, 0..100 â€” the millisecond snap-read, BEFORE any observer's taste.
 * Species-agnostic: symmetry, youth/vitality, presence, striking features â€” the model that
 ...
 * This is not a claim that only the young are beautiful â€” it's the default prior a stranger's
...
export function beautyOf(c: Identity): number {
  ...
  // youth/vitality curve: rises through adolescence, broad plateau ~18â€“32, gentle decline after.
  let v: number;
...
/** Map intrinsic beauty (0..100, 50â‰ˆordinary) to a signed baseline pull (-25..+45).
 *  This is the shared first read everyone gets before personal taste is applied. */
function beautyPull(beauty: number): number {
  // 50 â†’ ~0 (neutral). 75 â†’ ~+25 (turns heads). 90 â†’ ~+42. 30 â†’ ~-16 (off-putting).
 ...
 * caps at 5 (aesthetic appreciation without desire). Permissive when data is missing â€”
...
/**
 * First read â€” set once, the moment two people share a scene. Conditioning (taste) matched
...
export function seedAttraction(state: SaveState, fromId: string, toId: string): void {
  ...
  if (cap !== null && cap <= 5) {
    ...
    // BASELINE â€” the millisecond snap-read: intrinsic beauty, before personal taste. Beauty is
    // objective (symmetry/vitality is a given) and the body registers it fully regardless of clench.
    // Clench does NOT reduce the pull â€” a clenched nervous system wants the beautiful thing just as
    ...
    // PERSONAL DEVIATION â€” this observer's conditioning (taste) matched against what the target IS.
  ...
  e.attraction = a;
  e.attraction_base = a;
  // ADMISSIBILITY STAMP â€” born grasping or born in awe. Set once, from how clenched the perceiver
  // was at first sight. A pull that arises in a gripped body (low relaxation) is high-magnitude but
  // can't reach clean self-report: it will express as possession, sharpness, gift-that's-taking â€”
  ...
  // toward current relaxation each turn (tickDesire) â€” slowly up under calm, faster down under clench.
  if (a >= 12) {
 ...
 * permanently â€” a scar, aging, weight change, an ear blown off, being shrunk â€” their intrinsic
...
export function applyBeautyChange(state: SaveState, toId: string, oldBeauty: number, newBeauty: number): number {
  ...
  for (const e of state.world.edges) {
    ...
    // the conditioned FIRST read also drifts, but half as much â€” bedrock memory of the first sight.
...
export function desireLine(state: SaveState, id: string): string {
  ...
  if (a <= -15) return "desire toward you: none, actively repelled â€” SHOW: subtle withdrawal, stiffening at closeness; never narrate the aversion as a stated feeling";
  if (a < 15) return "desire toward you: none â€” SHOW: warmth stays platonic, a flirt would land awkward; do not invent attraction, and never narrate 'she felt nothing'";
  const adm = e.desire_admissibility ?? clamp(0.5 + r * 0.05, 0, 1);
  // Each line: a behavioral instruction (what to SHOW) plus an explicit NEVER â€” the narrator must not
  ...
  if (cold) return adm >= 0.4
    ? `desire toward you: strong (${a}), cold-natured â€” SHOW: patient charming pursuit, warmth deployed as a tool, gifts with strings; NEVER narrate the wanting or that the charm is technique â€” behavior only, let the player sense it`
    : `desire toward you: strong (${a}), cold and grasping â€” SHOW: possessiveness, tallying who's near you, sharpness toward rivals, a gift that's really a claim; NEVER narrate resentment, wanting, or "she resented not having him" â€” only the acts`;
  if (adm >= 0.6) return `desire toward you: real (${a}), settled â€” SHOW: flirts, teases, seeks closeness, lets you be; NEVER state the wanting outright â€” render it as behavior`;
  if (adm <= 0.35) return `desire toward you: strong (${a}) but unadmitted â€” SHOW: it leaks as grasping â€” possessiveness, sharpness, taking-for-your-own-good, a claim dressed as care; NEVER narrate the pull or that they can't admit it â€” only what they DO`;
  return `desire toward you: real (${a}), not yet settled â€” SHOW: surfaces in small glances and half-gestures when the moment allows; NEVER state it outright â€” behavior only`;
}
...
export function tickDesire(state: SaveState): string[] {
  ...
  for (const id of state.world.present) {
    ...
    const ceiling = base >= 15 ? 100 : 40; // flat first read â†’ companionate plateau
    ...
    if (!cond) continue;
    // ADMISSIBILITY DRIFT â€” the stamped grasp/awe texture migrates toward CURRENT relaxation, but
    ...
    // the target is higher than now; drops DOWN (âˆ’0.06/turn, ~3Ã— faster) when clench pulls it lower.
    ...
    if (e.attraction >= 45 && cond.psyche.relaxation <= -3 && !has) {
      ...
      shifts.push(`${c.name} is holding on too tight â€” wanting has turned into gripping.`);
    } else if (has && cond.psyche.relaxation >= 2) {
      cond.psyche.active_states = cond.psyche.active_states.filter((s) => s !== label);
      shifts.push(`${c.name}'s grip loosens â€” the wanting is still there, held lightly now.`);
    }
 ...
 *  past. Warmth (âˆ’100..100) is how much they CARE; trust (âˆ’100..100) is how much they RELY on you.
...
export function dispositionCue(warmth: number, trust: number): string {
  ...
  const care =
    warmth >= 70 ? "loves you / devoted (warmth very high) â€” open affection, protectiveness, seeks your closeness" :
    warmth >= 45 ? "is fond of you (warmth high) â€” visibly cares, softens around you, small kindnesses" :
    warmth >= 20 ? "likes you and is warming (warmth moderate, on a âˆ’100..100 scale where 0 is a stranger) â€” friendly, drawn to you, glad you're near" :
    warmth >= 5 ? "is mildly well-disposed (warmth slight) â€” cordial, no hostility" :
    warmth > -5 ? "is neutral (warmth ~0) â€” a stranger's baseline, neither warm nor cold" :
    warmth > -20 ? "is cool toward you (warmth mildly negative) â€” distant, unengaged" :
    warmth > -45 ? "dislikes you (warmth negative) â€” sharp, unwelcoming" :
    "resents or hates you (warmth very negative) â€” openly cold or antagonistic";
  const rely =
  ...
  const note = warmth >= 20 && trust < 20
    ? " â€” RENDER BOTH: the warmth is real and shows (care, softness, loyalty), the low trust only makes them guarded, NOT cold or hostile; do not write a caring character as a distant stranger"
    : warmth <= -20
      ? " â€” this coldness is THIS character's earned stance from what's passed between you, not a default suspicion to apply to everyone"
      : warmth >= 45 && trust >= 20
        ? " â€” let this warmth be plainly visible; do not make the player re-earn it every scene"
        : "";
