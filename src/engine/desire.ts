...
export function desireLine(state: SaveState, id: string): string {
  ...
  if (a >= 30 && e.warmth <= HOSTILE) {
    return adm <= 0.4
      ? `desire toward you: strong (${a}) AND THEY CANNOT STAND YOU (warmth ${Math.round(e.warmth)}) — both are true at once and NEITHER cancels the other. SHOW: they keep ending up where you are and are angry about it; they stand nearer than the argument needs; they touch you in ways that are not kind; contempt that keeps coming back for more of you; needling as a way of making contact; punishing you for a pull they will not own. NEVER soften this into flirtation, banter, or secret tenderness, never let it resolve into liking you, and NEVER narrate the wanting — the hostility is real, the wanting is real, and they are not a phase on the way to a bond.`
      : `desire toward you: real (${a}) while they dislike you (warmth ${Math.round(e.warmth)}) — both true, neither cancels the other, and they KNOW it about themselves. SHOW: frank, unsentimental appetite with no affection attached — they will say the cold thing and want you in the same breath, seek you out and give you nothing, be unembarrassed about the contradiction. NEVER write this as warmth breaking through, as a softening, or as banter that means they secretly care; do not make them nicer because they want you.`;
  }
  if (a >= 30 && e.warmth < COOL && !romantic) {
    return `desire toward you: real (${a}) with no attachment behind it (warmth ${Math.round(e.warmth)}) — they want you and have no particular feelings about you, which is a complete state and not an incomplete bond. SHOW: direct appetite without courtship — interest in your body and your presence, none in your day; they do not ask after you, do not soften, do not seek your company for its own sake, and are unbothered by whether you like them. NEVER render this as fondness, tenderness, or the beginning of caring, and never narrate the wanting outright — it is in what they do.`;
  }
