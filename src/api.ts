...
export const api = {
  ...
  /** PLAYER BASELINE TIGHTNESS — the persistent "running low today" the physiology clock can't see
   *  (unrested despite the hours, an off day, ambient stress outside the fiction). Maps a 0–5 body
   *  reading to a subjective relaxation ceiling that holds until cleared (level undefined). Level 0/1
   *  = no baseline limit (clears it); higher levels progressively cap how at-ease the body can get,
   *  stacking UNDER sleep/hunger. Reactive per-turn tightness is separate (passed to streamTurn). */
  setBaselineTightness: async (id: string, level: number | undefined): Promise<ClientSave> => {
    const s = await need(id);
    const pc = s.condition["char_player"];
    if (pc) {
      // 0/1 = ordinary, no baseline penalty (clear it); 2→+3, 3→0, 4→-3, 5→-5. Same shape as the
      // reactive anchor but living on the ceiling so it persists instead of decaying.
      const CAP: Record<number, number | undefined> = { 0: undefined, 1: undefined, 2: 3, 3: 0, 4: -3, 5: -5 };
      const lvl = typeof level === "number" ? Math.max(0, Math.min(5, Math.round(level))) : undefined;
      pc.subjective_ceiling = lvl === undefined ? undefined : CAP[lvl];
      await putSave(s);
    }
    return clientView(s);
  },
