    ...
    <div className="scroll-y h-full px-4 pb-10 pt-3 space-y-3">
      ...
      <div className="card p-4">
        ...
        <Toggle on={draft.habit_engine !== false} onFlip={() => setDraft((d) => ({ ...d, habit_engine: d.habit_engine === false ? true : false }))}
          title="Habit engine"
          desc="Core traits become firing habits with a frequency you can see and set: the share of scenes each pattern actually surfaces in, all the way up to every scene without exception. They loosen when a character sees themselves do them (clarity, not kindness), deepen when they don't, and give ground when somebody keeps pushing them on it — slowly at first, then faster. Change is directionless and never chosen; a character finds out they've changed when someone else notices. Set the dials in each character's drawer." />
