...
const SPARKS = [
  ...
  "A monastery that takes in anyone â€” and a stranger arrives at lamplight",
  ...
  const go = async () => {
    ...
    try {
      ...
      const seedThreads = chronicle.split("\n").map((l) => l.trim()).filter(Boolean).map((line) => {
        // "Title â€” description" or "Title: description" or just "Title"; leading "- " and "1." stripped
        const clean = line.replace(/^\s*(?:[-*â€¢]|\d+[.)])\s*/, "");
        const m = clean.match(/^(.*?)\s*(?:â€”|--|:)\s*(.+)$/);
  ...
  return (
    <div className="scroll-y h-full px-5 pb-10 pt-3">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <textarea className="field" rows={4} placeholder="A fishing village the winter the ice came earlyâ€¦"
        ...
        <div className="mt-5">
          <div className="font-mono text-[10px] uppercase tracking-wider mb-1.5" style={{ color: "var(--text-lo)" }}>
            Destination â€” where this story ends <span style={{ opacity: 0.6 }}>(optional)</span>
          </div>
          <textarea className="field" rows={2}
            placeholder="He learns to feed himself through winter and builds a shelter that holdsâ€¦"
        ...
        <div className="mt-4">
          <div className="font-mono text-[10px] uppercase tracking-wider mb-1.5" style={{ color: "var(--text-lo)" }}>
            Genre &amp; tone <span style={{ opacity: 0.6 }}>(optional â€” sets the register)</span>
          </div>
          <input className="field" type="text"
            placeholder={"e.g. action-horror survival, lethal and fast, romance under threat"}
            value={tone} onChange={(e) => setTone(e.target.value)} />
          <div className="text-[11.5px] leading-relaxed mt-1.5" style={{ color: "var(--text-lo)" }}>
            The key the whole story is written in. Set it and the world, threat, and prose are all built and rendered to match â€” no drifting into a quiet character study when you wanted horror. Leave blank to let the forge infer it from your seed.
          </div>
        </div>

        <div className="mt-4">
          <div className="font-mono text-[10px] uppercase tracking-wider mb-1.5" style={{ color: "var(--text-lo)" }}>
            Chronicle threads â€” story beats to seed <span style={{ opacity: 0.6 }}>(optional)</span>
          </div>
          <textarea className="field" rows={4}
            placeholder={"One beat per line. Title â€” optional detail.\nThe Rite of Voidbirth cult is smuggling artifacts through the Expanse\nAn old debt to Rogue Trader Vaicis comes due\nThe ship's Navigator is slowly going mad"}
            value={chronicle} onChange={(e) => setChronicle(e.target.value)} />
          <div className="text-[11.5px] leading-relaxed mt-1.5" style={{ color: "var(--text-lo)" }}>
            Set the plot points you want in play. Each becomes an active thread the world draws from, so what emerges is anchored to your intent instead of invented cold â€” the forge builds the cast and places so these are primed to happen. One per line; add "â€” detail" after a title to say more. You can always ignore them.
          </div>
        </div>

        <div className="mt-4">
          <div className="font-mono text-[10px] uppercase tracking-wider mb-1.5" style={{ color: "var(--text-lo)" }}>Forge model (OpenRouter id â€” pick the smith)</div>
          ...
          <button className="chip mt-2.5" onClick={() => setGrounded((v) => !v)}
            ...
            {grounded ? "â—‰" : "â—‹"} ground with web search
          </button>
        ...
        <motion.button className="btn btn-accent w-full mt-5" style={{ height: 50 }}
          ...
          <Hammer size={15} />
          {busy ? "forging â€” this takes a minuteâ€¦" : "Forge the world"}
        </motion.button>
        {busy && (
          <div className="font-mono text-[11px] text-center mt-3">
            <span className="shimmer">building the people, places, and stakesâ€¦</span>
          </div>
        )}
      </motion.div>
    </div>
