...
export default function Cast({ save, setSave, initialSel }: { save: ClientSave; setSave: (s: ClientSave) => void; initialSel?: string | null }) {
  ...
  return (
    <div className="scroll-y h-full px-4 pb-10 pt-3">
      ...
      {/* Sheet stays mounted across the open/close transition — that's what lets its internal
          AnimatePresence animate the panel out using the last real character data, the instant
          after `sel` (and so `c`/`cond`) has already gone back to null. */}
      <Sheet open={!!(c && cond)} onClose={() => setSel(null)} fill>
        {c && cond && (
          <>
            ...
            <div className="scroll-y px-5 pb-6 space-y-4">
              ...
              {editing && (
                <Section title="Edit">
                  <Field labelStyle="plain" label="Name" value={draft.name} onChange={(v) => setDraft((d) => ({ ...d, name: v }))} />
                  <Field labelStyle="plain" label="Age" value={draft.age} onChange={(v) => setDraft((d) => ({ ...d, age: v }))} />
                  <Field labelStyle="plain" label="Appearance — baseline (face, eyes, hair, build; the engine can only append here, never rewrite)" value={draft.appearance_facts} onChange={(v) => setDraft((d) => ({ ...d, appearance_facts: v }))} rows={3} />
                  ...
                  <Field labelStyle="plain" label="Presenting now (clothes, grime, visible state — freely rewritten in play)" value={draft.appearance_now} onChange={(v) => setDraft((d) => ({ ...d, appearance_now: v }))} rows={2} />
                  <div className="flex gap-2">
                    <Field labelStyle="plain" label="Height (ft)" value={draft.height_ft} onChange={(v) => setDraft((d) => ({ ...d, height_ft: v }))} rows={1} />
                    <Field labelStyle="plain" label="(in)" value={draft.height_in} onChange={(v) => setDraft((d) => ({ ...d, height_in: v }))} rows={1} />
                    <Field labelStyle="plain" label="Weight (lbs — scales hunger/thirst)" value={draft.weight_lb} onChange={(v) => setDraft((d) => ({ ...d, weight_lb: v }))} rows={1} />
                  </div>
                  <Field labelStyle="plain" label="Background — bedrock identity (never auto-trimmed)" value={draft.background} onChange={(v) => setDraft((d) => ({ ...d, background: v }))} rows={3} />
                  <Field labelStyle="plain" label="Story so far — what’s happened in play (auto-grows & compresses)" value={draft.life_history} onChange={(v) => setDraft((d) => ({ ...d, life_history: v }))} rows={3} />
                  <Field labelStyle="plain" label="Current goal" value={draft.current_goal} onChange={(v) => setDraft((d) => ({ ...d, current_goal: v }))} />
                  <Field labelStyle="plain" label="Core traits (one per line)" value={draft.core_traits} onChange={(v) => setDraft((d) => ({ ...d, core_traits: v }))} rows={4} />
              ...
              <Section title="Now" group="now">
                ...
                <KV k="mood" v={`${nice(cond.psyche.mood)} (${opennessLabel(cond.psyche.relaxation)})`} />
                {cond.psyche.active_states.length > 0 && <KV k="feeling" v={cond.psyche.active_states.map(nice).join(", ")} />}
                <KV k="body" v={`${nice(cond.fatigue)} · ${nice(cond.hunger)}`} />
                {cond.conditions.length > 0 && <KV k="afflicted" v={cond.conditions.map(nice).join(", ")} />}
                {cond.injuries.length > 0 && <KV k="injuries" v={cond.injuries.map((x) => nice(x.type)).join("; ")} />}
                {(c.texture ?? []).length > 0 && <KV k="texture" v={(c.texture ?? []).join(" · ")} />}
                {c.drive && <KV k="wants" v={`${c.drive.goal} — ${c.drive.progress}%${c.drive.blocker ? ` (blocked: ${nice(c.drive.blocker)})` : ""}`} />}
                {(c.drive_queue ?? []).length > 0 && <KV k="then" v={(c.drive_queue ?? []).map((d) => d.goal).join(" · ")} />}
                {c.appearance_now?.trim() && <KV k="presenting" v={c.appearance_now} />}
                {(cond.thirst_meter ?? 0) >= 6.5 && <KV k="thirst" v={(cond.thirst_meter ?? 0) >= 8 ? "parched" : "thirsty"} />}
                {(cond.awake_minutes ?? 0) >= 17 * 60 && <KV k="sleep" v={`${Math.round((cond.awake_minutes ?? 0) / 60)}h awake`} />}
                <KV k="where" v={save.world.places[c.location ?? ""]?.name ?? (c.location ? c.location : "—")} />
                {sel !== "char_player" && <KV k="status" v={c.tracked ? "followed — lives on in the world, always wanting something" : "not followed — fades into the background when offscreen"} />}
              </Section>
              ...
              {(cond.inventory.length > 0 || cond.wearing.length > 0) && (
                <Section title="Carrying & wearing" group="now">
                  {cond.wearing.length > 0 && <KV k="wearing" v={cond.wearing.map(nice).join(", ")} />}
                  {cond.inventory.length > 0 && <KV k="items" v={cond.inventory.map((i) => niceCap(i.name)).join(", ")} />}
                </Section>
              )}
    ...
    <Section title="Things going on in their life" group="now">
      {list.map((a, i) => (
        <div key={i} className="mb-3 pb-2" style={{ borderBottom: i < list.length - 1 ? "1px solid var(--ink-2)" : undefined }}>
          <KV k="doing" v={a.goal} />
          {a.approach && <KV k="by" v={a.approach} />}
          {a.because && <KV k="because" v={a.because} />}
          <KV k="how far" v={a.crystallized_turn
      ...
      {editing === null ? (
        ...
        <div>
          <Field labelStyle="plain" label="What they start doing" value={goal} onChange={setGoal} rows={2} />
          ...
          <Field labelStyle="plain" label="How they go at it (optional)" value={approach} onChange={setApproach} />
          <Field labelStyle="plain" label="Why it started — in their life, not yours (optional)" value={because} onChange={setBecause} rows={2} />
          ...
          <Field labelStyle="plain" label="Or: fully themselves within this many SHOWN turns" value={turns} onChange={setTurns} />
          <div className="text-[11px] mb-2" style={{ color: "var(--text-lo)" }}>
    ...
    <Section title={`Their week — it is ${WEEKDAY_FULL[today]}`} group="now">
      ...
      {blocks.map((b, i) => (
        <div key={b.id ?? i} className="mb-3 pb-2" style={{ borderBottom: i < blocks.length - 1 ? "1px solid var(--ink-2)" : undefined }}>
          ...
          <KV k="where" v={save.world.places[b.where]?.name ?? b.where} />
          {b.how && <KV k="gets there" v={b.how} />}
          {b.why && <KV k="because" v={b.why} />}
          {b.stakes && <KV k="or else" v={b.stakes} />}
          <KV k="how firm" v={b.paused ? "on hold" : RIGID.find((r) => r.k === b.rigidity)?.hint ?? b.rigidity} />
          {b.last_missed_day !== undefined && <KV k="missed" v={`day ${b.last_missed_day}`} />}
          <div className="flex flex-wrap gap-1.5 mt-2">
        ...
        <div>
          ...
          {(blocks.length > 0 || sched?.home || sched?.note) && (
            <div className="mt-3 pt-2" style={{ borderTop: "1px solid var(--ink-2)" }}>
              <Field labelStyle="plain" label="Where they end up otherwise (home)" value={sched?.home ?? ""} onChange={(v) => void run(() => api.setScheduleFrame(save.id, sel, { home: v }))} />
              <Field labelStyle="plain" label="Anything else about the week" value={sched?.note ?? ""} onChange={(v) => void run(() => api.setScheduleFrame(save.id, sel, { note: v }))} />
            </div>
          )}
        </div>
      ) : (
        <div>
          <Field labelStyle="plain" label="What they have to do" value={what} onChange={setWhat} />
          <div className="text-[11px] mb-2" style={{ color: "var(--text-lo)" }}>
          ...
          <Field labelStyle="plain" label="Where" value={where} onChange={setWhere} />
          <Field labelStyle="plain" label="How they get there (optional)" value={how} onChange={setHow} />
          <Field labelStyle="plain" label="Why it's in their life — theirs, not yours (optional)" value={why} onChange={setWhy} rows={2} />
          <div className="flex gap-2">
            <div className="flex-1"><Field labelStyle="plain" label="From" value={start} onChange={setStart} /></div>
            <div className="flex-1"><Field labelStyle="plain" label="Until" value={end} onChange={setEnd} /></div>
          </div>
          ...
          {rigidity === "mandatory" && (
            <Field labelStyle="plain" label="What missing it costs them" value={stakes} onChange={setStakes} />
          )}
          <div className="flex gap-1.5 mt-2">
