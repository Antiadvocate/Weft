...
- **Focus on an event (converge toggle)** — a button on the Play screen. When a main event is building (Zod arrives in two days) and you don't want the engine throwing fresh chaos that sidelines it, tap "focus on an event": new unrelated complications are suppressed and the narrator + simulator bend every scene toward the throughline, while the event itself still lands on schedule. Tap the X to release it and roam freely again. Off by default — you decide when the main thing is big enough to drive toward.
